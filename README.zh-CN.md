...
`/skills`（清单，`/技能名` 直接执行）、
`/open <@agent|#房间|@main>`（进入某个会话，Tab 从已存在的会话补全；
`Ctrl+K` 是同一扇门，不必打名字）、
...
**Esc 回 `@main`** —— 导航先于中断，所以背后正在跑的回合不受影响，它的
「Esc 中断」在 `@main` 等你。Ctrl+C 语义不变，在哪都能中断。
...
所以也不会因为你没看而丢失。回来时打一条 `── @main ──`，后面接 `@main` 在你离开期间完成的内容。

因为 scrollback 只写一次、绝不回改，来回几趟之后同一个会话会在屏幕上出现不止一次。
这是有意为之，分隔线会标出来；`Ctrl+O`（transcript 视图）仍是 `@main` 会话的完整记录。
...
的输出）。**main 也在花名册上，且排在第一位**——它和其他人一样是参与者，有自己的
会话、房间与记录——在它那一行按 Enter 去的是控制台，而不是某个 DM。↑/↓ 移动，
Enter 打开成员的会话或房间，`o` 打开成员的记录页，`j` 加入光标所在的房间。它只负责
导航与信息——停止 agent 仍归 Ctrl+B 管理器，而 main 根本不可停止。

**每个参与者都有自己的一页，main 也有。** 视角页是一份只读的两级档案，记录它说过
什么、又被交办过什么。索引把它的线程分组——合并的
`timeline`、按对端各一行的 `direct messages`、它所在的 `rooms`，以及它收到的
`intake`——每行带条数与时钟；Enter 进入线程，Esc 返回，`q` 关闭。线程读起来就是一
段对话，而这个参与者是它自己那一页的主角：无论对端是谁，它的思考与工具调用在**每
一个**线程里都显示。这是审计层——agent 与你之外的人的往来只在这里可见，而你和它的
DM 仍是纯粹的一对一，不会混入别的东西。它是快照：重新打开即刷新。

**三扇门通向它**：在会话里对着空输入框按 `tab`（`@agent` 里是那个 agent 的页，控制
台里是 main 的页；房间没有唯一主角，因此什么也不做）、在团队目录里对成员按 `o`、以
及在 Ctrl+B 管理器详情里对 agent 按 `tab`。main 的页由会话 transcript 构建——控制台
对话是它的 `@user` 泳道，每个给它写过信的 agent 各占一道，加上它的房间与作为 intake
的派发通知——而 transcript 上唯一的时钟是回合标记，所以页上的时间是回合时间，标记出
现之前录下的会话则没有时间。

**从没和你说过话的 `@agent` 会直说。** main 派出、你却从未去信的 agent，它的一对一
视图是空的——任务与汇报都属于 main——所以它开在规则线下多一条暗淡的
`· no conversation yet · tab opens @scout's record ·`，而不是一片空白。
