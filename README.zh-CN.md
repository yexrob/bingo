...
- 派生实例有名字（`name` 参数，缺省取定义名/`agent`，重名自动 `-2`/`-3`），
  ...
  生命周期事件不再写进 @main——改由 agent 树与 `Ctrl+B` 对话框里它自己那一行
  承载；完成后历史保留。
- `SendMessage` 向实例发后续指令（上下文保留）；实例忙时排队，当前回合结束
  自动送达。子代理的 `SendMessage(to: "main")` 落进主 agent 的收件箱，主 agent
  空闲时被唤醒，并在 @main 留下一行 `@scout❯ <摘要>`（发送者的身份色），
  完整正文只在 `ctrl+o` 记录视图里。摘要来自这次发送自己的可选 `summary` 字段——
  五到十个词，由发送方写，只出现在子代理的 schema 上，因为被画出来的正是子代理发的
  那一条——没写时退回消息自身的前五十列。房间转发什么都不画。
  `urgent: true`（仅子代理→main）在到达那一刻额外触发终端提醒通道。
- 运行**失败**时在 @main 画一行 `⚠ @名字 · 原因`并触发提醒；**完成**时在通知进入主
  agent 上下文的位置留下一行暗色 `● @名字 completed · 任务`；取消什么都不画。
...
**状态层**是回答「谁在干活」的那一层，就在输入框周围，从不写进流里。`Ctrl+T` 在
任务区 → agent 树 → 关闭之间轮换（名册为空时只在任务区 ↔ 关闭之间来回），Esc 一次
退一层。**agent 树**是 `@main` 加上每个实例一行，每帧从注册表重建、从不落进
scrollback：运行时 `@scout: reading src/lib.rs… · 12 tool uses · 8.3k tokens`，
等待时 `Idle for 14s`，停止后 `[stopped]`——名字各用自己的身份色，main 的那一格是
保留的。`Shift+↑/↓` 打开树并在其中移动：先是 `@main`（第一次按就停在那里），然后
按名字排序的实例，最后一行 `hide`，两端回绕。**选中是显式的**——没有选中时每个键
仍属于输入框——只有选中实例行时 `k` 才表示停止，走的是 `Ctrl+B` 对话框里 `x` 的同
一条路、同一句警告；`@main` 没有 `k`，对已停止的实例按 `k` 什么也不做。
`Ctrl+Shift+O` 在每行下面挂三行它自己会话的浓缩预览（终端不支持 kitty 键盘协议时
无法与 `Ctrl+O` 区分，按下去打开的是记录视图）。`Esc` 先清掉光标，再按一次才关闭
面板。选中一行按 `Enter` 就**放大**它——实例行打开那个 agent 的视图，`@main` 行离开
已打开的视图，`hide` 行收起面板；没有选中时 `Enter` 仍然是发送。树关着而名册上有人
时，窗口最后一行是**药丸条**：`@main @scout @writer · shift + ↓ to expand`，用身份
色，空闲或已停止的一律变暗，宽度不够时以 `→` 结尾。任务面板会把仍在名册上的负责人
标成 ` (@scout)`（用他的颜色），把被挡住的任务标成 ` › blocked by #3`——只是显示：
这里不指派、不认领、不解除阻塞。

**放大视图**把某一个 agent 的会话整屏放在你面前，想看多久就看多久。在树里选中一行
按 `Enter`，或在 `Ctrl+B` 对话框里按 `f`，屏幕就换成
`Viewing @scout · esc to return`，下面是它被派发时的任务，正文是这个 agent 的**完整
记录**，按顺序：创建它的任务、main 给它的指令、你自己发的消息、房间转发、提醒、它
的工作（折叠方式与 `@main` 相同，`⏺ Searched for 1 pattern, read 2 files`）、它的
回答，以及此刻正在流式产出的那一轮——不管是谁发起的。画它的是 `@main` 用的同一个
行构建器，所以一条消息在哪里读都是它自己的样子：同样的气泡、同样的 markdown、同样
的左侧装订线与头像、同样右对齐的 `HH:MM` 时间戳（不是今天的写作 `M/D HH:MM`，行太
窄时宁可丢掉时间戳也不折行），以及压在同一个人连续消息之上的名字——房间里超过两个
说话人时，名字不是装饰。

**输入框保持可用，并且指向这个 agent**，边框与 `❯` 都染上它的身份色：你打的字以你
自己的名义进它的收件箱，下一帧就以排队消息的样子出现在屏幕上，没有 `Sent to` 回执
——消息本身已经在屏幕上了。`/` 开头的一行是消息而不是命令，`!` 也是：控制台的解析
器根本看不到这份草稿。`PgUp`/`PgDn` 与滚轮翻页，视图跟着自己的尾巴走，直到你往上
翻。**`Esc` 有两重含义，按这个顺序**：先清掉树里的光标，再中止*正在运行*的那一轮
（历史保留，视图不关），最后才返回。`Shift+Tab` 轮换的是**这个 agent 的**权限模式，
不是 main 的。`Shift+↑/↓` 走名册，视图跟着走；树开着时走的是树，由 `Enter` 决定。
agent 离开名册时视图自己关闭；而一个*已完成*的 agent 的视图会留着，因为读它正是目
的。视图开着时 `Ctrl+O`、`Ctrl+B`、`Ctrl+T`、`Ctrl+G`、`Ctrl+R` 都是哑的，因为它们
要打开的东西没法盖在它上面。**房间**有同一套视图（房间的日志，含进出行；打字就是
发到房间，不是成员时先把你加进去），入口是 `Ctrl+B` 对话框里那一行的 `f`。
...
的同一份花名册是 `Ctrl+T` 的 agent 树，它以 `@main` 打头；对话框不列 main——它既不
...
**完成**且通知确实属于主 agent 时，多一行暗色 `● @scout completed · fix the parser`。
**消息**从 agent 来时，多一行 `@scout❯ <摘要>`，用发送者的颜色。**失败**多一行
`⚠ @scout · connection reset` 并触发提醒通道。除此之外——实例启动、转为空闲、被停
止、房间里的一条发言——什么都不写：状态是树的事，而每条房间发言都画一行正是消化去
抖存在要挡住的那种洪水。

**谁说了什么由一次遍历决定**，因为发送者不是一个字段：吸收进来的收件箱是一整段平铺
的提示词，能留下来的只有那些字面标记。放大视图保留这次遍历找到的每一个对端；未读计
