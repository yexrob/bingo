...
- 派生实例有名字（`name` 参数，缺省取定义名/`agent`，重名自动 `-2`/`-3`），
  ...
  生命周期事件不再写进 @main——改由编队生命周期日志、团队目录与该实例自己的记录页
  承载；完成后历史保留。
...
`· user joined · 14:32`。`/join #名字`（或在团队目录里对该房间按 `j`）是不说话地
成为成员，`/leave #名字` 是它的反面，退出后房间仍然可读。团队目录会列出每个房间，
...
房间与记录。↑/↓ 移动，`o` 打开成员的记录页，`j` 加入光标所在的房间。它只负责
...
**两扇门通向它**：在团队目录里对成员按 `o`，以及在 Ctrl+B 管理器详情里对 agent 按
`tab`。main 的页由会话 transcript 构建——控制台
