...
| `Ctrl+B` | 把正在前台运行的命令转入后台；没有命令在跑时打开后台对话框——agents / shells / rooms 三段（`Enter` 看详情，`f` 切到前台，`x` 停一个 agent，`tab` 打开它的记录页） |
| `Ctrl+L` | 清屏重画 |
...
- 派生实例有名字（`name` 参数，缺省取定义名/`agent`，重名自动 `-2`/`-3`），
  ...
  生命周期事件不再写进 @main——改由 agent 树、`Ctrl+B` 对话框里它自己那一行，
  以及该实例自己的记录页承载；完成后历史保留。
- `SendMessage` 向实例发后续指令（上下文保留）；实例忙时排队，当前回合结束
  自动送达。子代理的 `SendMessage(to: "main")` 落进主 agent 的收件箱，主 agent
  空闲时被唤醒，并在 @main 留下一行 `@scout❯ <消息前五十列>`（发送者的身份色），
  完整正文只在 `ctrl+o` 记录视图里；房间转发什么都不画。
...
前台没有命令在跑时按 `Ctrl+B` 打开**后台对话框**：**Agents** 是花名册和每个人正在
做的事，未读会以 `(3 unread)` 挂在行尾（点到你名字的那条用强调色）；**Shells** 是
后台命令及其状态；**Rooms** 是每个房间的成员与消息数，你不在的标 `you're not in`。
↑/↓ 跨段选择，`Enter` 看详情，`f` 把某个 agent 或房间切到前台（放大视图），
`x` 停掉正在跑的 agent，`←`/`Esc` 关闭。常驻版本的同一份花名册是 `Ctrl+T` 的
agent 树，它以 `@main` 打头；对话框不列 main——它既不可停止也无处可切，
它的会话就是你按下这个键时看着的那一屏。
...
**一扇门通向它**：在 `Ctrl+B` 对话框里对 agent 按 `tab`（`f` 打开的是放大视图，
那是活的会话而不是记录）。main 的页由会话 transcript 构建——控制台
