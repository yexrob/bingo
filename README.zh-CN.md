...
**状态层**是回答「谁在干活」的那一层——D115 之后，也回答「你没看的时候谁说了话」。
`Ctrl+T` 只开关任务区；树的门是 `Shift+↑/↓`。**agent 树**是 `@main` 加上每个实例
一行、再加上你在的每个房间一行，每帧从注册表重建、从不落进 scrollback：运行时
`@scout: reading src/lib.rs… · 12 tool uses · 8.3k tokens`，等待时 `Idle for 14s`，
停止后 `[stopped]`，房间是 `#dev-team: 3 members`——名字各用自己的身份色，main 的
那一格是保留的。每一行都戴着它那个会话的**徽章**：有未读是一个点（`•`），点名到
你——房间发言里 `@user`——是强调色的计数（`•3`）；进入即已读，读过徽章即清。
点名到你的发言还会在主流里留一行 `⚑ #dev-team @qa: <摘要>`——这是房间能得到的
唯一一行，每次点亮只留一次，读过房间才会再响；等你授权的 agent 行会变成强调色的
`waiting on you (permission)`，药丸同时挂旗，直到你答完弹窗。
`Shift+↑/↓` 打开树并在其中移动：先是 `@main`（第一次按就停在那里），然后按名字排
序的实例，然后房间，最后一行 `hide`，两端回绕。**选中是显式的**——没有选中时每个键
...
面板。选中一行按 `Enter` 就**放大**它——实例行打开那个 agent 的视图，房间行打开房间日志，
`@main` 行离开已打开的视图，`hide` 行收起面板；没有选中时 `Enter` 仍然是发送。树
关着而有人可列时，窗口最后一行是**药丸条**：`@main @scout •2 #dev-team •5 ·
shift + ↓ to expand`——在运行或有未读的亮起，点名到你的戴强调色计数，其余变暗，
宽度不够时以 `→` 结尾。任务面板会把仍在名册上的负责人
...
的同一份花名册是 `Shift+↑/↓` 的 agent 树，它以 `@main` 打头；对话框不列 main——它既不
