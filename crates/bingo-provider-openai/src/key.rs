...
mod tests {
    ...
    fn the_store_is_read_before_what_was_configured() {
        ...
        let key = ApiKey::new(
            "openai",
            places(Some("OPENAI_API_KEY")),
            store.clone(),
            configured("sk-settings", "OPENAI_API_KEY"),
        );
        ...
        store.remove("openai").expect("a removal");
        assert_eq!(
        ...
        assert_eq!(
            ...
            AuthStatus::Missing {
                hint: "No proxy1 key: run `/login proxy1` to paste a key, or configure \
                       openai.instances.proxy1.apiKey in settings."
                    .into()
            },
    ...
    async fn a_pasted_key_lands_in_the_store_and_a_logout_takes_it_out() {
        ...
        let key = ApiKey::new(
            ...
            places(None),
            store.clone(),
            configured("sk-settings", "openai.instances.proxy1.apiKey"),
        );
        let receipt = key
        ...
        assert_eq!(
            ...
            "Signed out of proxy1; openai.instances.proxy1.apiKey still applies."
        );
