...
name, without .md), description (one line), type (user | feedback | project \
| reference) — then the fact; feedback and project memories add **Why:** and \
**How to apply:** lines, and [[slug]] links another memory.
...
store what the repository already records.";
...
mod tests {
    ...
    /// Words this block may spend. Past ~120 it is a document, and a document
    /// in every prompt is a tax on every turn.
