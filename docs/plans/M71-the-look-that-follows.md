...
| does tmux answer `OSC 11` for its own pane, without passthrough? | documented to, from the ground it believes the outer terminal has | **not measured.** The risk the plan named — a tmux that answers with a *default* colour rather than the current one — is therefore still open, and the focus-time ask is the fallback |
