...
4. **Lanes in the live region and the tape** (worker) — a block (`ToolOutput.display`) is drawn under its tool row, folded like output (three rows, `… +N`), fully in the pager overlay; a panel (`Extension` that parses as a `View`) is drawn in `ctrl+t`; pinning is the TUI's, not the plugin's — `⏎` on a panel row in `ctrl+t` pins it into the live region under the running rows, remembered per session in `ui`; a signal is drawn in the live region under the running rows, newest last, at most 8 rows per plugin, the rest folded.
5. **Actions** (worker) — an `Actions` row draws `[ 1 Approve ] [ 2 Next ]`; when the panel or signal holding it has focus (`ctrl+t` then `⏎` on it, or the pinned one under `tab`), digits fire `Effect::Submit(Input::Action)`; the row shows `…` until the ack; a `Rejected` ack is a notice.
...
`crates/bingo-sdk/src/{command,event,state,host,testing}.rs`, `crates/bingo-core/src/{host,session,journal}.rs`, `crates/bingo-surface-rpc/src/{methods,server,client}.rs`, `schema/rpc.json`, `crates/bingo-tool-fs/src/{edit,write}.rs`, `crates/bingo-surface-print/src/render.rs`, `crates/bingo-surface-tui/src/views/*`, `src/{live,panel,block,input,keys}.rs`, new `crates/bingo-demo-ui/**`, `crates/bingo/src/main.rs`, `crates/bingo/tests/{cli/views.rs,views.rs}`.
