...
- [ ] a rostered holder hears every post `Hold` and runs no turn until
      something else opens one; absorption order is journal order
- [ ] `@parent` delivers `Wake` and opens a debt the seat's next post
      closes; the chaser and the card know the seat
- [ ] a holder never hears its own post; exactly-once fan-out still
      pinned with the holder counted
- [ ] a room without `parent` on the roster is byte-identical to today
- [ ] `WaitAgent` joins several agents under one deadline, outcomes per
      agent, error iff any is not an answer
- [ ] a never-run member reads as seated, never as finished
- [ ] the words teach both patterns; description tests pin them
