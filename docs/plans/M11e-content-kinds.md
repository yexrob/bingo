...
5. **Images** (worker) — `ratatui-image`: kitty and iTerm2 inline, sixel where probed, half-block cells otherwise; an `Asset` item draws a thumbnail (≤ 12 rows) in the transcript and opens full-size in a sheet; the probe runs once at start with a 400 ms cap; tmux passthrough where kitty is behind it. Measured against the budget before it is accepted; the fallback is `[image: name]` + `open` in the terminal's viewer.
...
8. **Rewind picker** (worker) — `esc esc` on an empty composer opens a card listing turns newest first; `⏎` submits `/rewind <turn>`; requires the store's rewind command (ADR-0005) — if absent the picker is not offered and the plan says so.
9. **Reasoning** (worker) — `⏎` on a focused `✻ thought for 3s` row opens the reasoning text in a sheet.

## Files

`crates/bingo-surface-tui/src/{markdown,transcript,composer,input,keys,theme,terminal}.rs`, new `highlight.rs`, `images.rs`, `complete.rs`, `sheets/{pager,rewind}.rs`, `Cargo.toml`, `scripts/budget.toml`, `deny.toml` if a licence needs listing.
...
- [ ] the pager sheet: open, search, close; the frame beneath byte-identical after
