...
6. **v1 mirrors Tool and Command only.** Hooks, contributors, providers, surfaces and stores stay in-process; the `Contribution` enum is the map of what a later version may cross, each gated on a real need. MCP remains the road for tool-only servers speaking MCP; the bridge is for bingo-native plugins.
