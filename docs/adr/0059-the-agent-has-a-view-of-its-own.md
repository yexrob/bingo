...
1. **One plugin, `bingo-persona`**, plugin tier, depending on the sdk and nothing else. It claims no settings key: `enabledPlugins["bingo.persona"] = false` (ADR-0057 §1) is its switch, and a plugin with one block and no knob needs no second one.
...
- No `/persona` command, no per-project text, no tone settings: the first real ask for one is the ADR that adds it.
