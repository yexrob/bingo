...
4. **What TOML cannot say.** No `null`: a write carrying one into a TOML layer is an error naming the key, and the tri-state of ADR-0003 §3 is available only from a JSON layer or `--settings <file>.json`. The one caller that hands one over is `/think off`, and it is not asking for the tri-state: `settings::remember(key, null)` means *this layer says nothing about this key*, so the key is removed rather than written — which in the user layer, the lowest and the only one a command writes, is the fact the `null` already stood for. A `null` deeper inside a written document is still the error. Comments do not migrate. A TOML datetime read into a layer arrives as whatever `toml_edit`'s deserializer makes of it; no setting is one.
