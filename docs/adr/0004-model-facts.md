...
4. **Declared overrides are a kernel settings key.** `models` (one of ADR-0003 §2's six kernel keys): `"<provider>/<model>": {contextWindow, maxOutput, reasoning, images}`, every field optional, objects deep-merged like any other key.
...
- **Amended 2026-09-21:** the session's `thinking` choice alone controls whether a reasoning parameter is requested. Model metadata (`reasoning`, including an explicit `false`) is informational, not permission to send effort. `/think` remembers the level in the user layer; `off` omits the parameter and leaves the server's default in place. Provider-specific effort mapping is unchanged.
