...
4. **Hooks reach every point.** The kernel now calls `on_compact(Before|After)` around a compaction, `on_session(Start)` when an actor opens and `End` when it closes, and `on_event(frame)` for every durable frame, on one ordered task per session that never holds the actor. `HookOutcome::Ask{reason}` from `before_tool` opens a permission interaction whose summary carries the reason.
