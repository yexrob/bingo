...
**CI matrix (`.github/workflows/ci.yml`).** Jobs: `fmt` (rustfmt check + `scripts/check_discipline.sh`); `test` on `ubuntu-latest, macos-latest` (`cargo check --workspace --all-targets --locked`, `clippy … -D warnings`, `cargo test --workspace --locked`); `windows-latest` required for `cargo check --workspace --all-targets --locked` and `continue-on-error: true` for its `cargo test` step, which is promoted to required at M6 with the shell dialect; `budget` (below); `tui-smoke` from M6. `Swatinem/rust-cache`, `--locked` everywhere.
