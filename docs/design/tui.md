...
| thinking | `✻ Thinking…` dim italic while it lasts, with **the tail of what is being thought under it** — the same three dim `⎿` rows a running tool shows (§6), scrolling up as they arrive, no fold and no key; after, `✻ Thought for 2s` (`<1s` under a second) with **what was thought under it** — dim, under a `⎿`, folded at five rows like a result and wearing the same `… +N lines (ctrl+o to expand)`; `ctrl+o` lifts the fold and again opens it whole, `⏎` on the focused row opens the sheet. A thought that came back empty (redacted, or a provider that summarised nothing) is the row alone: no tail, no fold, no key, no sheet |
...
| thinking | `✻ Thinking…` dim italic over the three dim `⎿` tail rows of what has been thought so far, scrolling up as the deltas arrive; then decays to `✻ Thought for 2s` — bringing the whole of it with it, folded — as the next block starts. The tail wears no comet: the glow is for words being said, and thinking is where `dim` lives (§4) | per delta |
...
- **Hands on the keys, the mouse welcome.** Every card answers to one key (`1-9`, `y/a/n`, `⏎`, `esc`); every sheet closes with `esc`; `?` shows the whole table; `ctrl+o` expands the latest fold — a result, a thought, an action's receipt, whichever is newest or focused — and again opens it whole; `ctrl+g` opens the switcher; `shift+tab` cycles the mode. The mouse scrolls, clicks a child's row or a card row, opens a fold, drags a selection; it is never required.
- **A key means one direction; a click means both.** `ctrl+o` only ever opens further (fold → whole → sheet) and `esc` comes back, because a key pressed twice must not undo itself. A **click on a block is one gesture on one row**: it opens what is folded under it and a second click folds it again, and the direction it takes is the one the row is not already in. It is the same fold either way — one set, filled from the keyboard or from the pointer — so a quiet notice, whose cut promises no key, still opens to a click.
