...
- **Sessions**: a child or peer is a row in the transcript where it began — `⏺ reviewer(review the manifest)` with `⎿  Running… 3 tools · 1.2k tokens` while it works, `⎿  Done (4 tools · 8.1k tokens · 40s)` after, `⎿  Needs you` pulsing — and `⏎` on that row steps into its transcript; the status line's right slot then reads `in reviewer`, and `ctrl+g` (the switcher: a dropdown above the input box, like the `/` menu) steps anywhere. A room's messages are `> reviewer: …` lines. There is no tab strip: a session that is doing nothing is not on screen.
...
| switcher | `ctrl+g` (as since M8; `ctrl+t` stays the panel sheet, Claude Code's todo key): a dropdown above the input box like the `/` menu — `❯ project ● · reviewer ⠹ needs you · #design` — `⏎` enters, `esc` closes |
