...
- **Sessions**: a child or peer is a row in the transcript where it began — `⏺ reviewer(review the manifest)` with `⎿  Running… 3 tools · 1.2k tokens` while it works, `⎿  Done (4 tools · 8.1k tokens · 40s)` after, `⎿  Needs you` pulsing — and `⏎` on that row steps into its transcript; the status line's right slot then reads `in reviewer`, and `ctrl+g` (the switcher: a dropdown above the input box, like the `/` menu) steps anywhere. A room's messages are `> reviewer: …` lines. **The quick cycle** (user-directed, 2026-08-31): `↓` on an empty composer turns the status line into a chip strip below the box — `● main  ○ reviewer  # design`, agents then rooms — and `←`/`→` switch the view live as the highlight moves; `↑`, `esc` or typing gives the line back. It appears only on the gesture, so the one-line-of-furniture rule holds; there is still no tab strip: a session that is doing nothing is not on screen until you ask to walk them.
