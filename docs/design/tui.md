...
| switcher | `ctrl+g` (as since M8; `ctrl+t` stays the panel sheet, Claude Code's todo key): a dropdown above the input box like the `/` menu — one session per row, `❯ reviewer ⏺ running · needs you`, the coloured `⏺` for the glance and the word for `NO_COLOR`; no title, no hint row — `⏎` enters, `esc` closes; more sessions than the dropdown has rows for scroll under the one the cursor is on, `…` at each end that was cut |
