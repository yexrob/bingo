...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  **Bottom entity area**: when running instances/channels exist, a one-line summary shows above the input box; idle/stopped agents stay out. Running rows include model/thinking/state. The area is a summary only — ↑/↓ belong to the composer's prompt history. Ctrl+G opens the fullscreen **Slack-shaped workspace** directly. Ctrl+B opens the main-view background-agent manager (↑/↓ select, Enter detail, x stop, Esc close) — unless a shell command is running in the foreground, in which case it backgrounds that command instead; detail shows prompt/status/elapsed/tokens/tool count/recent tool activity and opens that agent's DM with Enter. The workspace is one conversation pane, full width, rendering a Slack
...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  **Bottom entity area**: when running instances/channels exist, a one-line summary shows above the input; idle/stopped agents stay out, and running rows show model/thinking/state. The area is a summary only — ↑/↓ belong to the composer's prompt history. Ctrl+G opens the fullscreen **Slack-style workspace** directly. Ctrl+B opens the main-view background-agent manager (↑/↓ select, Enter detail, x stop, Esc close) — unless a shell command is running in the foreground, in which case it backgrounds that command instead; detail shows prompt/status/elapsed/tokens/tool count/recent tool activity and opens that agent's DM with Enter; hub-and-spoke has no foreground action. The workspace is the whole screen as a single message-flow column (a compact title at the top
