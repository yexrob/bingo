...
- Credentials come from `apiKey` in settings.json, the `ANTHROPIC_API_KEY` / `DEEPSEEK_API_KEY`
  environment variables (tried in that order; both are key sources for the default
  Anthropic-protocol endpoint — neither picks a provider), or a subscription login
  (`/provider login codex`). Custom endpoints use `ANTHROPIC_BASE_URL` or `apiBaseUrl`
  (settings take precedence over environment variables). Starting with none of them is fine:
  the welcome card shows onboarding, and the error comes only when a request is sent
  (see diagnostics 1).
...
- Shortcuts (press `?` with an empty input for the full table): Enter sends · `\`+Enter / Ctrl+J newline (multi-line input;
  ...
  Esc closes the topmost open layer first — permission dialog, picker, rewind, the `Ctrl+B` background dialog, slash or mention dropdown, ctrl+r search, error row, info rows,
  the `?` panel, the conversation rows, a manually opened task panel — and interrupts the turn when none of those is open (`@main`'s turn, and only at home:
  on a page Esc follows the page order below; bash mode exits below
  the interrupt, so Esc stops a running `!` command first), then double-press clears input · Ctrl+C busy interrupt (unconditional, layers do not shield it;
  once an interrupt has been ignored past its grace, the next Ctrl+C exits) / clears text /
...
Three config layers; a later layer overrides per key — with four exceptions: the `permissions`
lists and `disabledMcpServers` **accumulate** across layers (a local layer cannot drop a project
`deny`), `providers` merges **per provider name**, and the `experimental` switches latch on once
any layer sets them. `mcpServers` is replaced wholesale.
...
| `model` | string | The session's model at startup (persisted by `/model`); precedence: `--model` > settings > built-in default |
...
| `thinkingLevel` | string | Thinking level: `off` sends no thinking param (the default, and what an endpoint that refuses the parameter takes); `low`/`medium`/`high`/`xhigh`/`max` send `{"type":"adaptive"}` adaptive thinking plus `output_config.effort` (the Claude 5 family removed budget_tokens; below `high` saves tokens, `xhigh`/`max` think deeper) |
...
| `experimental` | object | Experimental features: `{"agentChannels": true}` enables agent room messaging (the main session and direct subagents get the Channel tool, and `SendMessage` accepts `#room` targets); `channelMessageLimit` (default 500, freezes the channel when exceeded) / `agentMessageLimit` (default 50) are budget gates; `{"chatAvatars": true}` turns on every avatar in the interface — the conversation gutter (portraits on image-capable terminals, initial-on-colour chips elsewhere) and a subagent's portrait on its watch row in place of `◉` (a row inside a grouped dispatch never wears one); default false = no avatars anywhere (identity colours are not avatars and stay) |
...
- **`bingo app-server`** — JSON-RPC 2.0, one JSON object per line (NDJSON) on
  ...
  writes the Draft-7 schema bundle (a manifest mapping every method to
  direction, params, result and declared errors — notifications to direction
  and params — plus a schema for each); the committed copy is in
  `schema/app-server`, and a client should generate its types from it rather
...
`/help` for the full list. Common ones: `/model [name]` (no args: two-level picker — level 1 providers → level 2 model list, which comes from the settings `models` declaration when there is one, otherwise the endpoint; `r` re-asks a fetched list; with a name: switch directly — an unknown name still switches, with an advisory `⚠ not in the known list` warning; switching is refused while a turn runs),
...
`/think [off|low|medium|high|xhigh|max]` (thinking level, persists to settings; no arg opens the level picker: ●=in effect, ↑↓/1-6 to browse, Enter persists, s = session-only, Esc cancels), `/theme [dark|light|auto]` (no arg opens the level picker; the `/theme auto` explicit shortcut stays),
...
`/context` (usage) · `/status` · `/config` (the effective configuration and which layer each value came from) ·
`/share [--public] [--open] [path]` (see Sharing below) · `/tasks` (the task list) ·
`/compact` (force compaction — of the console's session at home, of the agent whose page is on screen otherwise) · `/resume [name]` (restore a past session; no arg opens the session picker, Enter restores) · `/rename <name>` · `/gc` (clean expired session storage; 30-day TTL, latest 100 inactive sessions, 24-hour activity grace) · `/clear` · `/exit`.
Aliases: `/?` = help · `/quit` = exit · `/reset` and `/new` = clear · `/team` accepts `ls`/`st`/`check`/`up`/`down`/`say`.
...
2. **Model request fails/times out**: `/status` shows the current model; `/model` switches it; with multiple providers use
   ...
   when close to the context window, `/compact` (auto-compaction triggers at 90% of the effective window,
   the model's window minus its own output budget: 90% × (1M − 128k) ≈ 785k for current Claude models,
   90% × (1M − 384k) ≈ 554k for DeepSeek v4; family defaults live in `~/.config/bingo/model-catalog.json`, and a
   declared `maxTokens` sets the budget for any model, clamped to half the window). Endpoints without a
   ...
   an image a flat 1600), with a one-time stderr warning on first fallback.
   ...
   conversation.
   If an upstream response completes without assistant content or tool calls (an unclosed thinking block counts,
   unless the response was cut off by `max_tokens` — a truncated turn takes a resume prompt instead), bingo retries
   the side-effect-free attempt once with a warning; a second silence is taken as the model's decision, not a broken
   stream: the turn ends quietly — a warning row records the retried silence, nothing is written to history,
   and no error is raised. Transient error events received after a stream
...
3. **MCP server not working**: `/mcp` shows status — `✗ failed: <details>` fixes per the details
   ...
   (rewritten per connection; never printed into the UI); after fixing, `/mcp reconnect [name]` (all enabled servers when the name is omitted). `type: sse/ws` errors with "unsupported (stdio / http)".
...
4. **Permission prompts/denials not as expected**: `/permissions` lists the current rules; rule syntax is
   ...
   allow requires a **single rule covering all subcommands** to skip the prompt; `$()`/subshell/brace contents are
   split into segments that each need cover, and commands with unclosed quotes are never auto-allowed
   (`Bash(git log)` allows `git log` but not `git log | head`). File rules normalize paths before matching (`~` expansion, relative paths expanded against
...
8. **Tool output collapsed**: ctrl+o opens the transcript view — the whole session on its own screen with every
   ...
   view of its own.
9. **Slash dropdown doesn't have the command you want**: type a prefix to filter (e.g. `/m` matches mcp/model);
...
- **Built-in tools**: Bash (through the permission gate, combined output capped by `bashOutputMaxChars`),
  ...
  Agent (subagents), SendMessage (the one speech tool — see below), Channel (room lifecycle — main and direct subagents, behind `experimental.agentChannels`), AgentControl (subagent lifecycle, main session only),
...
- **Provider protocols**: anthropic (Messages API, default — all existing configs) and openai (Responses API,
  ...
  models work through the openai adapter; its chat/completions models need an adapter
...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  when no turn is running does not write into `@main` at all, and is carried instead by the
  conversation rows, the instance's row in the `Ctrl+B` dialog, and its own record.
  **What the dispatch row shows**: while the run is alive, the last three things the instance did hang
  ...
  **A completion leaves one dim `● @name completed · task` line** where the task notification reaches the main
  agent's context, before the main agent narrates anything — for a run this turn's own `Agent` call dispatched,
  and only for one: a delivery-triggered run (a room post or a queued message waking a member) still
  notifies main's context, but completes into the tree and the `Ctrl+B` dialog, never the flow.
  **A run failing is the one exception**: it draws one `⚠ @name · reason` alert line in `@main` and rings
  ...
  **digested on a debounce**: a burst buys one turn once the inbox has been quiet for two seconds (or after
  ...
  Room lines reach that inbox unconditionally: the main agent is a room member under the same
  rules as everyone else, and no member's inbox is gated — a line enters the mail when it is posted, named or
  not, and a debounce coalesces a burst into one digest turn rather than holding anything back.
  ...
  **Messaging**: `SendMessage` is the one way any participant speaks to any
  conversation, and `to` is the same conversation namespace the composer writes — an instance name or `@name`
  for an agent, `#name` for a room. Anyone with a name may write to anyone who has one: the main agent reaches
  any instance, and a subagent reaches `main` **and any named colleague directly** — a peer message arrives
  under a `[message from @name]` marker, stays private, and is answered with `SendMessage(to: "@name")` rather
  than in turn text (a colleague never reads your prose; it goes back to main as your result). What is still
  checked is the sender: a session with no name in the agent registry can only write to `main`, and a room
  requires membership. A subagent's message to `main` lands in main's inbox and starts a turn there if
  it is idle — and draws **nothing** in `@main`: the message is main's mail, not the user's
  conversation, and what the user sees is the sender's mail signal in the status layer until that agent's
  zoom is visited. What the user reads next is what main then says. The optional `summary` field (five to
  ten words, written by the sender) is accepted and carried but currently drives no surface — the `@name❯`
  line and the tree preview it once fed are retired — and it never enters the
  envelope. A **room** relay draws nothing at all: a room is a conversation between agents that main overhears, and
  ...
  other turn, in prose that renders in `@main` as main speaking and counts on its unread; the noise control is the wake debounce above and the dispatch row's own state.
  ...
  **Chasing a reply**: the harness does that polling for you, and it is on by default — every SendMessage **to an
  instance** arms a 300s check unless told otherwise (a subagent's message to `main` arms nothing and returns no
  `message_id` — main is not an instance in the registry; a room post ignores `ack_timeout`, its debt being the
  mention watchdog below). Once the wait elapses it re-reads the same record; while the sender is still owed an
  ...
  **Channel messaging** (experimental, `experimental.agentChannels`): main and direct subagents use the Channel
  tool to create channels and manage members (creating one seats the creator and nobody else; members are direct
  subagents plus `main` and `user`, each seated only when named), members speak
  ...
  over-budget channels auto-freeze and notify the main agent. **Delivery wakes, and nothing gates it**: a message lands in every member's inbox and
  whoever is idle starts a run on it — named or not — while a running member absorbs it at its next tool round, which costs input tokens and no model
  call. An empty inbox never wakes, so a quiet room costs nothing and nothing polls. The `@` decides what is *owed*, never what is read. Mentions
  that resolve to nobody, or name a stopped member, are reported back in the sender's tool result. **The `@` decides what a reply is owed**: each
  ...
  line "matters" is banned outright — a member that wakes on a batch owing nothing closes its turn with a line saying so (the turn text is
  what main reads; a turn that says nothing at all reports nothing at all) and stays out of the room. **An `@` on your name is a recorded
  debt**: it stays open until you post in that room, everyone can see you holding it — the conversation rows mark the member
  `owes #room #7`, the room `waiting on @dev · 3m` — and after five minutes unanswered a watchdog asks again, in the room, with the message
  quoted back. `@all` asks the room, not each member, and is owed one *covered* answer (a colleague already answering
  ...
  The main agent carries its own half of the etiquette (same gate): its room lines arrive as `[#room msg #N]` inside its mail envelope,
  it answers a room with `SendMessage(to: "#room")` rather than by talking at you, and it keeps you posted: when room lines reach it,
  its reply briefs you on what moved — in its own words, compressed, with the verbatim record on the room's page — rather than reading them in silence.
  **One transcript**: the terminal shows exactly one conversation — yours with the main agent — and the flow is that transcript in order. There is no conversation switching, no bar, no `/open`, no dividers and no replays; `Ctrl+K` is readline's kill-to-end-of-line again (`Alt+K` is its alias), and `↑/↓` belong to the composer's prompt history. **The one grammar that reaches somebody else is the composer's**: a line shaped `@name <message>` delivers the rest to that instance's inbox **as you**, bypassing the model entirely — an idle instance is woken, a running one takes it at its next tool round, and a stopped one is resumed from its kept history — and `#room <message>` posts to that room as `user`, joining you first (with the room's own membership line) if you are not a member. Success is a transient `Sent to @name` / `Sent to #room` above the composer: never a line in the flow, never in the model's history, gone at the next keystroke. **A name that resolves to nothing is prose** — `@utils explain this code` opens an ordinary turn, verbatim — so the grammar never swallows a question. The typeahead says which is which: at the start of a line `@` lists every instance as `@scout · send message · running` (stopped ones included, because a message resumes them) with project files under them, and `#` lists the rooms as `#build · post to room`, adding `· joins you` where you are not a member; mid-line the sigils are unchanged (`@` is the file-and-agent reference, `#` is a hash in a sentence). The conversation itself is one `Enter` away — see the pages below.
  **The conversation rows**: what exists, what is running and what has been said while you were not looking lives in a constant list under the composer, never in the flow. `● @main` leads it — filled while main works, its verb beside it — then every instance (`● @scout: reading src/lib.rs…` while it runs — the tool-use and token stats belong to the dispatch row and the `Ctrl+B` detail — `Idle for 14s`, `[stopped]`, or `owes #build #7` while it holds an unanswered mention), then the rooms you are in (`#dev-team: 3 members`, or `waiting on @dev · 3m` while a mention is open there), names in their identity colours, at most three rows with the cursor scrolling the window (`↓/↑ N more` on the edges). Every row wears its conversation's badge — unread is a bare dot (`•`), a post naming `@user` (or `@all`) is the count in the accent (`•3`) and rings once per turn-on (re-armed by reading the room; no flow line — the badge in constant view is the message); a subagent waiting on a permission answer turns its row to `waiting on you (permission)` in the accent. There is no key to learn: `↓` at the bottom of your prompt history falls onto the rows (CC's own fallthrough), `↓/↑` walk them, `Enter` opens the row's page, `k` stops a selected running instance (same path and warning as the `Ctrl+B` dialog's `x`), `Esc` or `↑` off the top returns to the composer, and any printable character gives the keyboard straight back to the draft. The **task panel** (`Ctrl+T`) names an owner who is still on the roster as ` (@scout)` in their colour, and marks a blocked task ` › blocked by #3` — display only: nothing here assigns, claims or unblocks.
  **What the transcript shows of an agent's life**: four tiers and no more. A **dispatch** is `◉ @scout: fix the parser` with the last three things the agent did condensed under it while it runs — or one `In progress… · 4 tool uses · 8.3k tokens` line where the window is too short for them — and several `Agent` calls from one round draw one tree (`⏺ Running 2 agents…`, `├─ @scout: fix the parser · 1 tool use · 2.1k tokens`), which dissolves back into individual rows the moment you expand a member. When the run ends the row **settles** to `Done (12 tool uses · 8.3k tokens · 1m 4s)` and that is what reaches scrollback; the live progress was never stored anywhere it could. A **completion** adds one dim `● @scout completed · fix the parser` line before main says anything about it — only for a run this turn's own `Agent` call dispatched. A **failure** keeps its `⚠ @scout · connection reset` alert and the attention channel with it. Everything else — running, idle, stopped, cancelled, a room post, a message an agent sends main — writes nothing at all: state is the tree's business, mail is main's, and a line per room post is the flood the digest debounce exists to prevent.
  **The pages**: `Enter` on a conversation row — or `f` in the `Ctrl+B` dialog — turns the screen to *that conversation's page*, drawn by the very pipeline `@main` is drawn by: the same markdown, bubbles, avatar gutter, collapse folds and stamps, flushed into the terminal's own scrollback as it settles (switching is a page turn: the page you leave is banked, the next starts at the top; coming home reprints a recent tail of main's). An agent's page opens with `── @scout ──` and is its **whole record** in order — the task, main's instructions, your messages, room relays, its folded work, its answers, and the run streaming right now. Typing *prose* on a page is a message to its subject, as you: a stopped instance is resumed by it, and the echo is the receipt. Every direct message shows on the receiver's page **when it is sent** rather than when the receiver reads it — yours, main's instructions to it, another agent's mail — so a busy instance's page tells you what it has been asked for before its next tool round absorbs it. **The console's own grammar keeps working there**: `/` is a command, `!` is shell mode, and `@name <message>` / `#room <message>` reach somebody who is not the page's subject — only prose follows the page, and the command tiers answer on whatever page you are looking at. A command acts on the **console's** session wherever it is typed, because that is what a console command is, with one exception: `/compact` on an agent's page compacts **that agent's** context (refused while its turn is running — `Esc` stops it first), and on a room's page says there is no context to compact. A command typed while main's turn runs waits behind that turn as it always has, and says `queued behind main's turn` when you are not on main's page to see the queue. A **room's page is speech only** — what members `SendMessage`d to the room, each post under its sender's name and face; membership changes stay in the log and off the page; typing posts to the room and joins you first.
  **`Esc` on a page has four meanings, in this order**: a *running* subject's turn is stopped first (its history is kept and the page stays), then shell mode is left if it is armed (it is the console's and survives a page turn), then a non-empty draft is cleared, then the page comes home — and main's own turn is out of Esc's reach while a page is up (`Ctrl+C` keeps the override). `Shift+Tab` cycles **that agent's** permission mode, not main's, and the footer badge follows it. A page closes itself when its subject leaves the registry; a *finished* agent's page stays open, because reading it is the point. `Ctrl+O` follows the active page; `Ctrl+B`, `Ctrl+T` and the rest keep working — a page is not a modal.
  **The background dialog**: `Ctrl+B` — with no shell command running in the foreground — opens one modal over everything working in the background, and a second `Ctrl+B` (or `Esc`, or `←`) closes it. `Background tasks`, the running counts under it, then the sections that have anything in them: **Agents** (`@scout: reading src/lib.rs…`, its identity colour, `(3 unread)` where its conversation has moved since you read it — in the accent where it said your name), **Shells** (`$ cargo build (running)`, the chip green on `done`, red on `error`), and **Rooms** (`#build: 12 messages · main, scout`, marked `you're not in` where you are not a member). A heading only appears where there is another kind to tell it apart from, and an empty dialog says `No tasks currently running`. The bottom line is `↑/↓ to select · Enter to view · f to foreground · x to stop · ←/Esc to close`, with `f` and `x` shown only where the row can be asked for them. Rows are ordered running-first and then by what moved most recently, and the cursor follows *its row* rather than a position, so `x` cannot stop something that slid under it. `f` foregrounds — the conversation's page, for an agent or a room; `x` stops a running instance (one warning, no confirmation; a background shell cannot be stopped and says so); `Enter` opens a detail that replaces the list (an instance's activity, cost, progress and prompt; a shell's status, runtime, command and output tail; a room's members, count and last words), where `←` goes back and `Esc`/`Enter`/`Space` close — `← to go back · Esc/Enter/Space to close`, plus `x` and `f` where they apply.
  **Rooms**: a **room** is the only group conversation there is, and its members are any subset of the team — it does not have to include you, and agents form rooms among themselves with the `Channel` tool (creating one seats the creator and nobody else; `user` and `main` are seated only when named). You speak in a room with `#room <message>` from the composer; if you are not a member that posts you in, and joining and leaving are **written into the room where every member sees them**, as dim `· user joined · 14:32` lines, so there is no quiet way to lurk and then speak. `/join #room` makes you a member without saying anything, and `/leave #room` is its counterpart; a room you leave stays readable. Roster changes never wake anybody and never make a serial sender stale.
  **The team is not a conversation** — it is a roster and a room list, and both live in the `Ctrl+B` dialog: the `Agents` section is who exists and what each one is doing, the `Rooms` section is every room with its members, its message count and a `you're not in` mark on the ones you are not in. The conversation rows under the composer are the always-on version of the same roster, and they lead with `@main`, which the dialog does not list — the console is neither stoppable nor foregroundable, and its conversation is the screen you pressed the key on. `/join #room` joins a room without speaking; posting from a room's zoom joins you first. What just happened is on the flow's own dispatch and completion rows, and `/team` answers on the info tier; the agent's page is the live record, and there is no second, read-only copy of it to open.
  **Attribution**: one walk recovers who said what from an agent's own history, because the sender is not a field — an absorbed inbox is one flat prompt and only its literal markers survive. Both readers of that walk agree by construction: an agent's **page** keeps every counterpart, and the **unread accounting** keeps exactly one, the pair of you and the agent. So the task it was created with, main's instructions to it, room relays, mail from other agents, the chases for its silence and the task reminder are all somebody else's conversation and count as nothing of yours; its work comes back the way `@main`'s does — one collapsed activity group per run (`⏺ Searched for 4 patterns, read 2 files`) hung on the agent's own message, not a column of `⏺ Tool(…)` lines; and the `[DM from user]` marker is transport rather than text and never renders. The default flips with the protagonist: in a subagent's record unmarked prose is main speaking, in main's own record it is you, and main was never dispatched so there is no spawn task to file as intake.
  **Unread**: the count is messages, not history rows, so a turn that made forty tool calls counts as one reply; it wears the mention accent only when the agent actually answered you, and it is drawn in the `Ctrl+B` dialog's chip (`(3 unread)`), where entering the conversation clears it. `@main` has a count of its own — main speaking while a view has the screen counts, and a `⚠ @name · …` failure alert counts *and* wants you. A room says your name case-insensitively (`@User`, `@USER,` and `@all` reach you; `@username` does not).
  **Avatars** (`experimental.chatAvatars`, off by default — the one switch every avatar follows; off = no gutter, chips or watch-row portraits anywhere, and identity colours stay because a colour is not an avatar): with it on, on terminals that can place kitty images (the same capability that renders inline images), each sender gets one of eight bundled anime-style portraits, 4×2 cells beside the name; elsewhere it falls back to the sender's initial on a colour, and the row count is identical either way. A team member's portrait is pinned in `.bingo/team.json` (`"avatar": "sora"`), so a crew keeps a fixed cast; everyone else gets a face derived from their name. **Every conversation wears the face in a left gutter, `@main` included** — four or five cells taken out of the width before the body wraps, the portrait on the first row of each sender's run and blank on every row after it, with work steps and system lines taking the indentation and no face. The main agent has a **reserved portrait**: it is not in the vocabulary `team.json` may pin and the name hash never lands on it, so the console looks the same in every session and no teammate can be mistaken for it. A terminal that purges its image store (a resize) redraws the faces still on screen; rows already in scrollback leave blank columns where the portrait was. The watch row wears that agent's portrait instead of its `◉` under the same switch — a row inside a grouped dispatch wears neither, because the stem and the name in its identity colour already say who it is.
...
- **rooms and the team tree**: a team declares its rooms in `channels[]`, each with its own mode, budget and roster — a department
...
- **crew first, hires temporary**: where a crew is pinned, it is the default workforce — work goes to a member by SendMessage,
...
- **Sessions**: transcripts persisted (JSONL), `--continue`/`/resume` restore, `/compact` compacts. A session's transcript file is opened when the session starts, in every frontend, so it can be listed, resumed and renamed before anything has been said in it; `--continue` therefore restores the most recent session that was actually *used*, and an empty one left by a launch-and-quit is reaped by `/gc`. Startup cleanup and `/gc` enforce a 30-day TTL plus a latest-100 inactive-session cap plus a 24-hour activity grace; share snapshots are removed with their transcript, while prompt-history files use the same TTL and a 100-file cap. `/cd <dir>` switches the session-owned working directory without changing the process cwd; subsequent Bash/Read/Edit/Write/Glob/Grep calls, project skills/agent definitions, Team/Agent crew lookup, Experience project keys, memory extraction, settings command paths, image paths, and `/team` resolve from the new directory. Startup-loaded settings/MCP configuration and the already-built system prompt are not reloaded.
  **Sharing**: `bingo share [session]` generates a self-contained HTML file in the current directory by default (`--output`
  ...
  `/share [--public] [--open] [path]` uses the same safety semantics: local by default, only `--public` uploads.
  The session key has the same semantics as `/resume` (transcript stem or a matchable fragment); without one it
  defaults to the most recent session that was actually *used* — an empty launch-and-quit transcript is skipped,
  the same filter as `--continue`.
  **Updates**: `bingo update` pulls the latest release from GitHub Releases (yexrob/bingo) and atomically replaces the current
  ...
  then settle and stay; a keypress settles it early; `motion: "off"` or `BINGO_NO_MOTION` shows it statically).
