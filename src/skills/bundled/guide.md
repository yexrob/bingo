...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  team lifecycle log, the team directory's roster, and the instance's own record.
  ...
  **The perspective page (D96, every participant since D100)**: one participant's communications, read-only, from its own point of
  ...
  shows none at all. **Two doors** reach the page: `o` on a member in the `Ctrl+T` team directory, and `tab` on an agent in the
  `Ctrl+B` manager's detail. (D103 retired the third, `tab` on an empty composer, with the conversation it was scoped to; `tab` in
  the composer is completion again, and `Enter` in either surface is deliberately unbound until the zoomed view lands.) It is a two-level page on the alternate screen, like the `Ctrl+O` transcript, with the same
