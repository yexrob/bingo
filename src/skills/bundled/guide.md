...
`/compact` (force compaction — of the console's session at home, of the agent whose page is on screen otherwise) · `/resume [name]` (restore a past session; no arg opens the session picker, Enter restores) · `/rename` · `/gc` (clean expired session storage; 30-day TTL, latest 100 inactive sessions, 24-hour activity grace) · `/clear` · `/exit`.
...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  **Messaging (D98)**: `SendMessage` is the one way any participant speaks to any conversation, and `to` is the
  ...
  ten words, written by the sender) still feeds the tree's `Ctrl+Shift+O` preview and never enters the
  ...
  **The conversation rows (v6, replacing D104's tree and pills)**: what exists, what is running and what has been said while you were not looking lives in a constant list under the composer, never in the flow. `● main` leads it — filled while main works, its verb beside it — then every instance (`● @scout: reading src/lib.rs… · 12 tool uses · 8.3k tokens` while it runs, `Idle for 14s`, `[stopped]`), then the rooms you are in (`#dev-team: 3 members`), names in their identity colours, at most three rows with the cursor scrolling the window (`↓/↑ N more` on the edges). Every row wears its conversation's badge — unread is a bare dot (`•`), a post naming `@user` is the count in the accent (`•3`) and rings once per turn-on (re-armed by reading the room; no flow line — the badge in constant view is the message); a subagent waiting on a permission answer turns its row to `waiting on you (permission)` in the accent (D116). There is no key to learn: `↓` at the bottom of your prompt history falls onto the rows (CC's own fallthrough), `↓/↑` walk them, `Enter` opens the row's page, `k` stops a selected running instance (same path and warning as the `Ctrl+B` dialog's `x`), `Esc` or `↑` off the top returns to the composer, and any printable character gives the keyboard straight back to the draft. The **task panel** (`Ctrl+T`) names an owner who is still on the roster as ` (@scout)` in their colour, and marks a blocked task ` › blocked by #3` — display only: nothing here assigns, claims or unblocks.
