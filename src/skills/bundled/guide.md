...
`/open <@agent|#room|@main>` (enter a conversation; Tab completes from the ones that exist, and it runs mid-turn; Ctrl+K is the same door without typing a name) ·
...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  **Conversations (D89)**: one terminal, one flow, one conversation at a time. `@main` (you and the main agent), a DM with a running subagent and a room are all the same shape and share one composer, one key surface, one approval path and one transcript renderer — there is no separate screen and no second set of controls. **Entering** one is `/open @agent` / `/open #room` / `/open @main` (Tab completes from the conversations that exist; the command is instant, so it works mid-turn), Enter on an agent in the Ctrl+B manager, or Enter on a member or a room in the team directory. **Esc returns to `@main`** — navigation before interruption, so a running turn survives the press and its Esc-to-interrupt is reachable from `@main`; Ctrl+C still interrupts from anywhere. **Switching** stashes the draft you were writing in the conversation you left, restores that conversation's own draft, prints a `── @name ──` / `── #name ──` rule into the flow and replays that conversation's last eight messages; from then on only it prints live. Everything else keeps accumulating in the store it already lives in and counts up an unread badge — nothing is buffered on the user's behalf. Returning prints `── @main ──` followed by whatever `@main` finished while they were away. Because scrollback is written once and never rewritten, repeated excursions leave a conversation on screen more than once; the rules mark it, and Ctrl+O (the transcript view) is still the complete record of the `@main` session. **Sending** goes to the active conversation: a DM delivers to that instance under the user's name (same immediate dispatch path as SendMessage, shown at once and marked pending only until the receiver claims it), a channel posts as `user` (the same delivery path an agent's room message takes, members woken normally; rendering counts as read, so serial never bounces the user), and a room posts as `user` only if you are a member of it — a room you are merely watching refuses and says `read-only · /join to speak in this room`. **None of these start a model turn**, and a DM message never steers or queues behind one. Slash commands are the exception and act on the application from any conversation. A DM in flight shows its state in the redrawn tail — the sent message, the agent's tool/reasoning steps as dim lines, and a `⠙ name is replying…` row carrying the instance's live token rate — and none of that reaches scrollback, because only the record is printed. **Conversation chrome (D90)**: a one-row conversation bar on the window's last row, below the composer (D97), lists the conversations you are in (`@main`, your rooms, DMs) with presence for DMs, unread counts, a distinct emphasis when a conversation named the user, and the active one accented; it renders only when more than one conversation exists and elides to `…` around the active entry when the width runs out. `Ctrl+K` opens the switcher over every conversation — recency order with `@main` pinned, fuzzy-filtered as you type, Enter switches, `Ctrl+X` stops the highlighted agent through the Ctrl+B manager's own stop path, Esc peels only the switcher — and it is inert behind a permission dialog. From **`@main` only**, a message opening with a known conversation's name (`@agent ` / `#channel `, matched exactly against the registry) delivers the rest there without switching and leaves a dim `→ @name: …` receipt in the flow; an unknown name is ordinary prose and starts a normal turn. Inside a conversation there is no line-leading routing, because the buffer already is the target. `/team` output goes to the team directory's recent feed rather than main's info tier, and main says `→ team (ctrl+t)` unless the directory is already open. While a DM is active the composer's border and `❯` glyph, and the `is replying…` row, wear that agent's palette colour.
...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  **Channel messaging** (experimental, `experimental.agentChannels`): the main agent uses the Channel tool
  ...
  in a turn woken by a channel message, the body text goes back to main — **only a message addressed to the room can put words in it**. The rule lives in the system block, not the wake payload: compaction rewrites message history but never touches the system prompt,
