...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  **Channel messaging** (experimental, `experimental.agentChannels`): the main agent uses the Channel tool
  ...
  ↑/↓ belong to the composer's prompt history; what exists and what is running is the agent tree's job, not a strip's. Ctrl+B opens the main-view background-agent manager (↑/↓ select, Enter detail, x stop, Esc close) — unless a shell command is running in the foreground, in which case it backgrounds that command instead; detail shows prompt/status/elapsed/tokens/tool count/recent tool activity, and opens its perspective page with tab (`Enter` is unbound since D103 — see the perspective page below).
  **One transcript (D103)**: the terminal shows exactly one conversation — yours with the main agent — and the flow is that transcript in order. There is no conversation switching, no bar, no `/open`, no dividers and no replays; `Ctrl+K` is readline's kill-to-end-of-line again (`Alt+K` is its alias). **The one grammar that reaches somebody else is the composer's**: a line shaped `@name <message>` delivers the rest to that instance's inbox **as you**, bypassing the model entirely — an idle or stopped instance is resumed, a running one takes it at its next tool round — and `#room <message>` posts to that room as `user`, joining you first (with the room's own membership line) if you are not a member. Success is a transient `Sent to @name` / `Sent to #room` above the composer: never a line in the flow, never in the model's history, gone at the next keystroke. **A name that resolves to nothing is prose** — `@utils explain this code` opens an ordinary turn, verbatim — so the grammar never swallows a question. The typeahead says which is which: at the start of a line `@` lists every instance the send can reach as `@scout · send message · running` (stopped ones included, because a message resumes them) with project files under them, and `#` lists the rooms as `#build · post to room`, adding `· joins you` where you are not a member; mid-line the sigils are unchanged (`@` is the file-and-agent reference, `#` is a hash in a sentence). **Until the zoomed view there is no surface that shows an agent's conversation**: its record is one key away (`tab` in the Ctrl+B detail), the agent tree's `Ctrl+Shift+O` preview shows three condensed lines of it, and `Enter` is deliberately unbound everywhere.
  ...
  **The perspective page (D96, every participant since D100)**: one participant's communications, read-only, from its own point of
  ...
  shows none at all. **One door** reaches the page for now: `tab` on an agent in the
  `Ctrl+B` manager's detail. (D103 retired the composer's `tab` door with the conversation it was scoped to and `tab` there is
  completion again; D104 took `Ctrl+T` away from the directory, whose `o` was the other door; `Enter` is deliberately unbound
  until the zoomed view lands.) It is a two-level page on the alternate screen, like the `Ctrl+O` transcript, with the same
...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  **The status layer (D104)**: `Ctrl+T` cycles `tasks → the agent tree → closed` — `tasks ↔ closed` when nobody is on the roster, and Esc closes one stop. The **agent tree** is `@main` plus one row per instance, rebuilt from the registry every frame and never written into scrollback: `@scout: reading src/lib.rs… · 12 tool uses · 8.3k tokens` while it runs, `Idle for 14s` while it waits, `[stopped]` once stopped — names in their own identity colours, main's reserved. `Shift+↑/↓` opens the tree and walks it: `@main` first (that is where the first press parks), then the instances in name order, then a `hide` row, wrapping at both ends. **Selection is explicit** — with nothing selected every key still belongs to the composer — and only a selected instance row makes `k` mean stop, which goes through the same path and the same warning the `Ctrl+B` manager uses; `@main` has no `k` and is not stoppable, and `k` on a stopped instance does nothing. `Ctrl+Shift+O` hangs three condensed lines of each instance's own conversation off its row (terminals without the kitty keyboard protocol cannot tell it from `Ctrl+O` and get the transcript instead). `Esc` clears the cursor first and closes the panel on the second press. `Enter` is unbound until the zoomed view. With the tree closed and anybody on the roster, the window's last line is the **pills**: `@main @scout @writer · shift + ↓ to expand`, identity colours, dim where an instance is idle or stopped, `→` where the window is too narrow for all of them. The **task panel** names an owner who is still on the roster as ` (@scout)` in their colour, and marks a blocked task ` › blocked by #3` — display only: nothing here assigns, claims or unblocks.
  **The team is not a conversation** — it is a directory: the roster with presence and the rooms each member is in, every room with its members and a `you're not in` mark, and the last ten lifecycle events (spawn/done, plus `/team` output, which also prints on the info tier). **Main is on the roster, first** (D100): it is a participant like the rest — presence is the host turn, its label is `main · console · idle`, its rooms list like anyone's. It navigates and informs only — stopping an agent stays in the `Ctrl+B` manager, which owns that verb and its warning. **It has no door between D104 and the background dialog**: `Ctrl+T`'s second stop is the agent tree now, and the dialog that opens the directory is the next batch's.
  **The perspective page (D96, every participant since D100)**: one participant's communications, read-only, from its own point of view — a two-level page on the alternate screen with the transcript's scrolling and `/` search, grouping a merged `timeline`, `direct messages` one row per counterpart, `rooms`, and `intake` (Enter opens a thread, Esc walks back, `q` closes). **Main has one too**, built from the session transcript: the console conversation as its `@user` lane, one lane per agent that wrote to it, its rooms, its dispatch notifications as intake; the attribution default flips with the protagonist (unmarked prose is main in a subagent's record, you in main's), and a transcript's only clock is the turn marker, so times there are turn times and a pre-marker session shows none. **One door for now**: `tab` on an agent in the `Ctrl+B` detail (D103 retired the composer's `tab` door with the conversation it was scoped to, D104 took `Ctrl+T` away from the directory, and `Enter` is unbound until the zoomed view lands). It is a snapshot: reopening is the refresh, and nothing on it can be typed into.
  In a DM or a channel the sender's name heads each run of messages (grouped: consecutive messages from one speaker carry it once), and every message carries its send time on its own first row, right-aligned and muted, at least two columns clear of the text — `HH:MM`, dated `M/D HH:MM` once it is not from today — the same stamp the main transcript uses; where the row is too narrow to hold both, the stamp is dropped rather than the message wrapped. DM replays reuse the main transcript's user bubbles, assistant markdown, prefixes, wrapping and row structure, because they *are* the main transcript's row builder.
  **The pure pair (D99)**: an agent's own record separates what *you* said to it and what it answered *you* from everything else it has been told. The task it was created with, main's instructions to it, room relays, mail from other agents, the chases for its silence and the task reminder are all somebody else's conversation and live in their own lanes on its perspective page. Its work comes back the way @main's does — one collapsed activity group per run (`⏺ Searched for 4 patterns, read 2 files`) hung on the agent's own message, not a column of `⏺ Tool(…)` lines. The `[DM from user]` marker is transport rather than text and never renders. Since D103 there is no surface that shows that pair on its own; the perspective page is where an agent's conversations are read until the zoomed view lands.
  **Unread (D99)**: a DM's badge counts messages, not history rows, so a turn that made forty tool calls counts as one reply; it wears the mention accent only when the agent actually answered you. `@main` has a badge of its own — main speaking while you are in another conversation counts, and a `⚠ @name · …` failure alert counts *and* wants you. A room says your name case-insensitively (`@User`, `@USER,` reach you; `@username` does not).
  **Avatars**: on terminals that can place kitty images (the same capability that renders inline images), each sender gets one of eight bundled anime-style portraits, 4×2 cells beside the name; elsewhere it falls back to the sender's initial on a colour, and the row count is identical either way. A team member's portrait is pinned in `.bingo/team.json` (`"avatar": "sora"`), so a crew keeps a fixed cast; everyone else gets a face derived from their name. **Every conversation wears the face in a left gutter** (D97, extended to `@main` in D99) — four or five cells taken out of the width before the body wraps, the portrait on the first row of each sender's run and blank on every row after it, with work steps and system lines taking the indentation and no face. The main agent has a **reserved portrait**: it is not in the vocabulary `team.json` may pin and the name hash never lands on it, so the console looks the same in every session and no teammate can be mistaken for it. A terminal that purges its image store (a resize) redraws the faces still on screen; rows already in scrollback leave blank columns where the portrait was. The sender name still heads each run of messages in a DM or a room, because with more than two speakers the name is not decoration; @main has no name rows, because it has exactly two. `experimental.chatAvatars` (off by default) governs one remaining thing: whether a subagent's watch row wears that agent's portrait instead of its `◉`.
