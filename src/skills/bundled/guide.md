...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  **Messaging (D98)**: `SendMessage` is the one way any participant speaks to any conversation, and `to` is the
  ...
  it is idle, and leaves **one** line in `@main` (D106): `@scout❯ <the message's first fifty columns>` in the
  sender's identity colour, with its send time. The whole body is one keystroke away, in the `Ctrl+O` transcript
  ...
  **Named limits**: `Ctrl+O`, `Ctrl+B`, `Ctrl+T`, `Ctrl+G` and `Ctrl+R` are inert while a view is open, because the surfaces they open cannot be shown over it. A **room** has the same view (the room's log, membership lines included, typing posts to it and joins you first), and no door until the background dialog opens one.
