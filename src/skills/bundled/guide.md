...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  **Channel messaging** (experimental, `experimental.agentChannels`): the main agent uses the Channel tool
  ...
  ↑/↓ belong to the composer's prompt history; what exists and what is running is the conversation bar's job, not a second strip's. Ctrl+B opens the main-view background-agent manager (↑/↓ select, Enter detail, x stop, Esc close) — unless a shell command is running in the foreground, in which case it backgrounds that command instead; detail shows prompt/status/elapsed/tokens/tool count/recent tool activity, opens that agent's DM with Enter, and opens its perspective page with tab.
  ...
  **The team is not a conversation** — it is a directory, on the second stop of `Ctrl+T` (tasks → team → closed; Esc closes one stop). It shows the roster with presence and the rooms each member is in, every room with its members and a `you're not in` mark, and the last ten lifecycle events (spawn/done, plus `/team` output). ↑/↓ move, Enter opens a member's DM or a room, `j` joins the room under the cursor; chords pass through, so `Ctrl+T` closes it and `Ctrl+K` still reaches the switcher. It navigates and informs only — stopping an agent stays in the `Ctrl+B` manager, which owns that verb and its warning.
  **The perspective page (D96)**: any agent's communications, read-only, from that agent's own point of view. `tab` on an agent in
  the `Ctrl+B` manager's detail opens it — a two-level page on the alternate screen, like the `Ctrl+O` transcript, with the same
  ...
  Today an agent's counterparts are `user` and `main`, because agents reach each other through rooms rather than direct messages.
