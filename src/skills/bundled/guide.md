...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  **Channel messaging** (experimental, `experimental.agentChannels`): the main agent uses the Channel tool
  ...
  ↑/↓ belong to the composer's prompt history; what exists and what is running is the agent tree's job, not a strip's. Ctrl+B opens the **background dialog** (D107) — unless a shell command is running in the foreground, in which case it backgrounds that command instead. It is one modal over everything working in the background: `Agents`, `Shells` and `Rooms`, `↑/↓ to select · Enter to view · f to foreground · x to stop · ←/Esc to close`, with a second `Ctrl+B` closing it. `f` opens that agent's — or that room's — **zoomed view** (D105); `x` stops a running instance; `Enter` opens a detail (an instance's activity, cost, progress and prompt; a shell's status, runtime, command and output tail; a room's members, count and last words), where `←` goes back, `Esc`/`Enter`/`Space` close, and `tab` opens the instance's perspective page.
  ...
  **Named limits**: `Ctrl+O`, `Ctrl+B`, `Ctrl+T`, `Ctrl+G` and `Ctrl+R` are inert while a view is open, because the surfaces they open cannot be shown over it. A **room** has the same view (the room's log, membership lines included, typing posts to it and joins you first), reached by `f` on its row in the `Ctrl+B` dialog.
  ...
  **The perspective page (D96, every participant since D100)**: one participant's communications, read-only, from its own point of
  ...
  shows none at all. **One door** reaches the page: `tab` in the `Ctrl+B` dialog, on an agent's row or in its detail —
  where `f` is the zoomed view, which is the live conversation rather than the record.
  (D103 retired the composer's `tab` door with the conversation it was scoped to and `tab` there is completion again;
  D104 took `Ctrl+T` away from the team directory, whose `o` was the other door, and D107 retired the directory itself.) It is a two-level page on the alternate screen, like the `Ctrl+O` transcript, with the same
...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  **Named limits**: `Ctrl+O`, `Ctrl+B`, `Ctrl+T`, `Ctrl+G` and `Ctrl+R` are inert while a view is open, because the surfaces they open cannot be shown over it. A **room** has the same view (the room's log, membership lines included, typing posts to it and joins you first), reached by `f` on its row in the `Ctrl+B` dialog.
  **The team is not a conversation** — it is a roster and a room list, and since D107 both live in the `Ctrl+B` dialog: the `Agents` section is who exists and what each one is doing, the `Rooms` section is every room with its members, its message count and a `you're not in` mark on the ones you are not in. The agent tree (`Ctrl+T`) is the always-on version of the same roster, and it leads with `@main`, which the dialog does not list — the console is neither stoppable nor foregroundable, and its conversation is the screen you pressed the key on. `/join #room` joins a room without speaking; posting from a room's zoom joins you first. The D95 team directory and its lifecycle feed are retired: what just happened is on the flow's own dispatch and completion rows, and `/team` answers on the info tier.
  **The perspective page (D96, every participant since D100)**: one participant's communications, read-only, from its own point of view — a two-level page on the alternate screen with the transcript's scrolling and `/` search, grouping a merged `timeline`, `direct messages` one row per counterpart, `rooms`, and `intake` (Enter opens a thread, Esc walks back, `q` closes). **Main has one too**, built from the session transcript: the console conversation as its `@user` lane, one lane per agent that wrote to it, its rooms, its dispatch notifications as intake; the attribution default flips with the protagonist (unmarked prose is main in a subagent's record, you in main's), and a transcript's only clock is the turn marker, so times there are turn times and a pre-marker session shows none. **One door**: `tab` in the `Ctrl+B` dialog, where `f` is the zoomed view — the live conversation rather than the record (D103 retired the composer's `tab` door with the conversation it was scoped to, D104 took `Ctrl+T` away from the team directory, and D107 retired the directory itself). It is a snapshot: reopening is the refresh, and nothing on it can be typed into.
  In a DM or a channel the sender's name heads each run of messages (grouped: consecutive messages from one speaker carry it once), and every message carries its send time on its own first row, right-aligned and muted, at least two columns clear of the text — `HH:MM`, dated `M/D HH:MM` once it is not from today — the same stamp the main transcript uses; where the row is too narrow to hold both, the stamp is dropped rather than the message wrapped. DM replays reuse the main transcript's user bubbles, assistant markdown, prefixes, wrapping and row structure, because they *are* the main transcript's row builder.
  **The pure pair (D99)**: an agent's own record separates what *you* said to it and what it answered *you* from everything else it has been told. The task it was created with, main's instructions to it, room relays, mail from other agents, the chases for its silence and the task reminder are all somebody else's conversation and live in their own lanes on its perspective page. Its work comes back the way @main's does — one collapsed activity group per run (`⏺ Searched for 4 patterns, read 2 files`) hung on the agent's own message, not a column of `⏺ Tool(…)` lines. The `[DM from user]` marker is transport rather than text and never renders. The **zoomed view** (D105) shows the agent's *whole* record rather than that pair, and the perspective page is where the lanes are read apart.
  **Unread (D99)**: a DM's badge counts messages, not history rows, so a turn that made forty tool calls counts as one reply; it wears the mention accent only when the agent actually answered you. `@main` has a badge of its own — main speaking while you are in another conversation counts, and a `⚠ @name · …` failure alert counts *and* wants you. A room says your name case-insensitively (`@User`, `@USER,` reach you; `@username` does not).
