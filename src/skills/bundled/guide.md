...
2. **Model request fails/times out**: `/status` shows the current model; `/model` switches it; with multiple providers use
   ...
   stream (D124): the turn ends quietly — a warning row records the retried silence, nothing is written to history,
...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  **A completion leaves one dim `● @name completed · task` line** where the task notification reaches the main
  ...
  and only for one (D114): a delivery-triggered run (a room post or a queued message waking a member) still
  ...
  **digested on a debounce**: a burst buys one turn once the inbox has been quiet for two seconds (or after
  ...
  Room lines reach that inbox unconditionally (v7): the main agent is a room member under the same
  ...
  **Messaging (D98; addressing widened by D137)**: `SendMessage` is the one way any participant speaks to any
  ...
  it is idle — and draws **nothing** in `@main` (D114): the message is main's mail, not the user's
  ...
  line and the tree preview it fed retired with D114/v6 — and it never enters the
  ...
  **Channel messaging** (experimental, `experimental.agentChannels`): main and direct subagents use the Channel
  ...
  over-budget channels auto-freeze and notify the main agent. **Delivery wakes, and nothing gates it (v7)**: a message lands in every member's inbox and
  ...
  call. An empty inbox never wakes, so a quiet room costs nothing and nothing polls. The `@` decides what is *owed*, never what is read (v6's five-unread
  and two-minute gates are gone: the count was an amplifier — in a room of six, one round where everyone speaks re-crossed it for all of them). Mentions
  that resolve to nobody, or name a stopped member, are reported back in the sender's tool result. **The `@` decides what a reply is owed (D119, sharpened by D128)**: each
  ...
  debt (D131)**: it stays open until you post in that room, everyone can see you holding it — the conversation rows mark the member
  ...
  it answers a room with `SendMessage(to: "#room")` rather than by talking at you, and it keeps you posted (D123): when room lines reach it,
