...
- Shortcuts (press `?` with an empty input for the full table): Enter sends · `\`+Enter / Ctrl+J newline (multi-line input;
  ...
  start and Alt+K to line end (Alt+K was Ctrl+K until the switcher took that key) · Ctrl+Y paste back the newest deletion and Alt+Y right after it cycles the 10-entry kill ring (consecutive
  ...
  while one is open) · Ctrl+T cycles the task area, then the team directory (roster · rooms · recent), then closed · Ctrl+G (or the readline chord Ctrl+X Ctrl+E) composes the current draft in `$VISUAL`/`$EDITOR`: the draft goes to a temp file, the editor gets the terminal, and the saved content replaces the input as one undo step; a non-zero exit keeps the draft and says so, an editor that exits having written nothing says so too (an editor that opens its own window needs its wait flag — `code -w` — or it returns before you have typed and its edit is read back as no edit at all), and with neither variable set the info tier says `set $EDITOR to edit the prompt in your editor`. Composing while a turn runs is fine; a permission dialog keeps priority. A conversation is entered with Ctrl+K (the switcher: every conversation in one list, most recently active first with the hub pinned on top, type to filter, Enter opens, Ctrl+X stops the highlighted agent through the same path the Ctrl+B manager's `x` uses, Esc closes without switching; inert while a permission dialog is up), with `/open <@agent|#room|hub>`, or with Enter on an agent in the Ctrl+B manager, and Esc returns to the hub (navigation before interruption — the turn keeps running). A conversation bar above the composer lists what exists (presence for DMs, unread counts, the active one accented) and renders only when there is more than one conversation · Ctrl+B moves the shell command running in the foreground to the background (same process, same output, returns a task id and notifies on completion), and with none running manages running background agents · Ctrl+L clear and redraw · Shift+Tab cycles permission
...
| `experimental` | object | Experimental features: `{"agentChannels": true}` enables agent room messaging (the main session and direct subagents get the Channel tool, and `SendMessage` accepts `#room` targets); `channelMessageLimit` (default 500, freezes the channel when exceeded) / `agentMessageLimit` (default 50) are budget gates; `{"chatAvatars": true}` puts a subagent's portrait on its watch row in place of `◉` (default false; the avatar gutter every conversation wears is not governed by it) |
...
`/open <@agent|#room|hub>` (enter a conversation; Tab completes from the ones that exist, and it runs mid-turn; Ctrl+K is the same door without typing a name) ·
`/join [#room]` · `/leave [#room]` (become a member of a room, or stop being one; with no argument they mean the room you are standing in) ·
...
9. **Slash dropdown doesn't have the command you want**: type a prefix to filter (e.g. `/m` matches mcp/model/meye);
   ...
   `/open` offers the conversations that exist, with their unread counts.
...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  **A run failing is the one exception (D98)**: it draws one `⚠ @name · reason` alert line in the hub and rings
  the attention channel, because bad news must not depend on the main agent choosing to narrate it; done and
  cancelled draw nothing. **A run the user triggered themselves** — everything in the batch that woke it came from
  their own DM — ends with no task notification and no woken turn for the main agent at all; the team directory's
  feed still records it, because the directory is a record. Room and direct mail for the main agent is
  ...
  **Messaging (D98)**: `SendMessage` is the one way any participant speaks to any conversation, and `to` is the
  conversation namespace the bar already uses — an instance name or `@name` for an agent, `#name` for a room.
  ...
  **Channel messaging** (experimental, `experimental.agentChannels`): the main agent uses the Channel tool
  ...
  reports to the hub, so **only a message addressed to the room puts words in it** — a reply written as turn text reaches nobody in the channel. It lives in the system block on purpose: compaction rewrites the history
  but never touches the system prompt, so the rule survives a long-running member's context being summarised away.
  ↑/↓ belong to the composer's prompt history; what exists and what is running is the conversation bar's job, not a second strip's. Ctrl+B opens the main-view background-agent manager (↑/↓ select, Enter detail, x stop, Esc close) — unless a shell command is running in the foreground, in which case it backgrounds that command instead; detail shows prompt/status/elapsed/tokens/tool count/recent tool activity, opens that agent's DM with Enter, and opens its perspective page with tab (one of the three doors — see the perspective page below).
  **Conversations (D89)**: one terminal, one flow, one conversation at a time. The hub (you and the model), a DM with a running subagent and a room are all the same shape and share one composer, one key surface, one approval path and one transcript renderer — there is no separate screen and no second set of controls. **Entering** one is `/open @agent` / `/open #room` / `/open hub` (Tab completes from the conversations that exist; the command is instant, so it works mid-turn), Enter on an agent in the Ctrl+B manager, or Enter on a member or a room in the team directory. **Esc returns to the hub** — navigation before interruption, so a running turn survives the press and its Esc-to-interrupt is reachable from the hub; Ctrl+C still interrupts from anywhere. **Switching** stashes the draft you were writing in the conversation you left, restores that conversation's own draft, prints a `── @name ──` / `── #name ──` rule into the flow and replays that conversation's last eight messages; from then on only it prints live. Everything else keeps accumulating in the store it already lives in and counts up an unread badge — nothing is buffered on the user's behalf. Returning prints `── hub ──` followed by whatever the hub finished while they were away. Because scrollback is written once and never rewritten, repeated excursions leave a conversation on screen more than once; the rules mark it, and Ctrl+O (the transcript view) is still the complete record of the hub session. **Sending** goes to the active conversation: a DM delivers to that instance under the user's name (same immediate dispatch path as SendMessage, shown at once and marked pending only until the receiver claims it), a channel posts as `user` (the same delivery path an agent's room message takes, members woken normally; rendering counts as read, so serial never bounces the user), and a room posts as `user` only if you are a member of it — a room you are merely watching refuses and says `read-only · /join to speak in this room`. **None of these start a model turn**, and a DM message never steers or queues behind one. Slash commands are the exception and act on the application from any conversation. A DM in flight shows its state in the redrawn tail — the sent message, the agent's tool/reasoning steps as dim lines, and a `⠙ name is replying…` row carrying the instance's live token rate — and none of that reaches scrollback, because only the record is printed. **Conversation chrome (D90)**: a one-row conversation bar on the window's last row, below the composer (D97), lists the conversations you are in (hub, your rooms, DMs) with presence for DMs, unread counts, a distinct emphasis when a conversation named the user, and the active one accented; it renders only when more than one conversation exists and elides to `…` around the active entry when the width runs out. `Ctrl+K` opens the switcher over every conversation — recency order with the hub pinned, fuzzy-filtered as you type, Enter switches, `Ctrl+X` stops the highlighted agent through the Ctrl+B manager's own stop path, Esc peels only the switcher — and it is inert behind a permission dialog. From the **hub only**, a message opening with a known conversation's name (`@agent ` / `#channel `, matched exactly against the registry) delivers the rest there without switching and leaves a dim `→ @name: …` receipt in the flow; an unknown name is ordinary prose and starts a normal turn. Inside a conversation there is no line-leading routing, because the buffer already is the target. `/team` output goes to the team directory's recent feed rather than the hub's info tier, and the hub says `→ team (ctrl+t)` unless the directory is already open. While a DM is active the composer's border and `❯` glyph, and the `is replying…` row, wear that agent's palette colour.
  **Rooms and the team directory (D95)**: a **room** is the only group conversation there is, and its members are any subset of the team — it does not have to include you, and agents form rooms among themselves with the `Channel` tool (creating one seats the creator and nobody else; `user` and `main` are seated only when named). A room you are in is one of your conversations: it is in the bar, in `Ctrl+K`, and you speak in it by typing. A room you are **not** in is still visible — the team directory lists it, marked `you're not in`, and Enter opens it **read-only**: the same flow, framed `── #name · observer · read-only ──`, with a standing `read-only · /join to speak in this room` hint under the composer and a refusal (not an error) if you submit anyway. `/join` — or `j` on the room in the directory — makes you a member; joining and leaving are **written into the room where every member sees them**, as dim `· user joined · 14:32` lines, so there is no quiet way to lurk and then speak. `/leave` is the counterpart, and a room you leave drops out of the bar while staying readable. Roster changes never wake anybody and never make a serial sender stale.
  **The team is not a conversation** — it is a directory, on the second stop of `Ctrl+T` (tasks → team → closed; Esc closes one stop). It shows the roster with presence and the rooms each member is in, every room with its members and a `you're not in` mark, and the last ten lifecycle events (spawn/done, plus `/team` output). **Main is on the roster, first** (D100): it is a participant like the rest — presence is the host turn, its label is `main · console · idle`, its rooms list like anyone's — and Enter on its row goes to the console, not to a DM with an instance that does not exist. ↑/↓ move, Enter opens a member's conversation or a room, `o` opens the member's perspective page, `j` joins the room under the cursor; chords pass through, so `Ctrl+T` closes it and `Ctrl+K` still reaches the switcher. It navigates and informs only — stopping an agent stays in the `Ctrl+B` manager, which owns that verb and its warning, and main is not stoppable at all.
  **The perspective page (D96, every participant since D100)**: one participant's communications, read-only, from its own point of
  view — **main has one too**, built from the session transcript: the console conversation as its `@user` lane, one lane per agent
  that wrote to it, its rooms, and its dispatch notifications as intake. The attribution default flips with the protagonist: in a
  subagent's record unmarked prose is main speaking, in main's own record it is you, and there is no spawn task to file as intake.
  A transcript's only clock is the turn marker, so times on main's page are turn times, and a session recorded before those markers
  shows none at all. **Three doors** reach the page: `tab` on an **empty composer** inside a conversation (the agent's page in
  `@agent`, main's in the console; a `#room` has no single protagonist, so the key does nothing there — with text in the composer
  `tab` is still completion), `o` on a member in the `Ctrl+T` team directory, and `tab` on an agent in the `Ctrl+B` manager's
  detail. It is a two-level page on the alternate screen, like the `Ctrl+O` transcript, with the same
  scrolling and the same `/` search. The **index** groups that agent's threads: a merged `timeline`, its `direct messages` one row
  per counterpart, the `rooms` it speaks in, and `intake` — the task that created it, its reminders and the chases for its silence —
  each with a message count and when it last moved. Enter opens one thread, Esc walks back to the index, `q` closes the page.
  A thread reads as a conversation, and the agent is the **protagonist of its own page**: its thinking rows and tool calls are shown
  in *every* thread, whoever the counterpart is, exactly as its DM shows them. A room thread is the whole room's log with that
  agent's own rows marked, not a monologue. **This is the audit layer**, and it is the one place an agent's conversations with
  someone other than you are visible — your own `@agent` DM never mixes them in. It is a **snapshot**: it is built when it opens and
  does not move while you read, so reopening is how you refresh. Nothing on it can be typed into; there is no composer and no send path.
  Today a subagent's counterparts are `user` and `main`, because agents reach each other through rooms rather than direct messages;
  main's counterparts are `user` and every agent that has written to it.
  In a DM or a channel the sender's name heads each run of messages (grouped: consecutive messages from one speaker carry it once), and every message carries its send time on its own first row, right-aligned and muted, at least two columns clear of the text — `HH:MM`, dated `M/D HH:MM` once it is not from today — the same stamp the main transcript uses; where the row is too narrow to hold both, the stamp is dropped rather than the message wrapped. DM replays reuse the main transcript's user bubbles, assistant markdown, prefixes, wrapping and row structure, because they *are* the main transcript's row builder.
  **`@agent` is the pure pair (D99)**: what you said, what it answered *you*, and the work it did for those turns — nothing else. The task it was created with, main's instructions to it, room relays, mail from other agents, the chases for its silence and the task reminder are all somebody else's conversation and live on its perspective page; they no longer render here as dim scaffolding lines. Its work comes back the way @main's does — one collapsed activity group per run (`⏺ Searched for 4 patterns, read 2 files`) hung on the agent's own message, not a column of `⏺ Tool(…)` lines. And the live tail belongs to **your** run: when the agent is working for main, for a room or on a chase, the DM shows no stream and no `is replying…` row, because no reply is owed to you. The `[DM from user]` marker is transport rather than text and never renders. An agent main spawned that you never wrote to therefore has an **empty** pair view, and it says so (D100): the conversation opens with its rule plus one dim `· no conversation yet · tab opens @scout's record ·`, which is not repeated while it is already the last thing that conversation printed.
  **Unread (D99)**: a DM's badge counts messages, not history rows, so a turn that made forty tool calls counts as one reply; it wears the mention accent only when the agent actually answered you. `@main` has a badge of its own — main speaking while you are in another conversation counts, and a `⚠ @name · …` failure alert counts *and* wants you. A room says your name case-insensitively (`@User`, `@USER,` reach you; `@username` does not).
  **Avatars**: on terminals that can place kitty images (the same capability that renders inline images), each sender gets one of eight bundled anime-style portraits, 4×2 cells beside the name; elsewhere it falls back to the sender's initial on a colour, and the row count is identical either way. A team member's portrait is pinned in `.bingo/team.json` (`"avatar": "sora"`), so a crew keeps a fixed cast; everyone else gets a face derived from their name. **Every conversation wears the face in a left gutter** (D97, extended to the hub in D99) — four or five cells taken out of the width before the body wraps, the portrait on the first row of each sender's run and blank on every row after it, with work steps and system lines taking the indentation and no face. The main agent has a **reserved portrait**: it is not in the vocabulary `team.json` may pin and the name hash never lands on it, so the console looks the same in every session and no teammate can be mistaken for it. A terminal that purges its image store (a resize) redraws the faces still on screen; rows already in scrollback leave blank columns where the portrait was. The sender name still heads each run of messages in a DM or a room, because with more than two speakers the name is not decoration; @main has no name rows, because it has exactly two. `experimental.chatAvatars` (off by default) governs one remaining thing: whether a subagent's watch row wears that agent's portrait instead of its `◉`.
...
- **Subagents**: instances spawned by Agent have names (the `name` arg, defaulting to the definition name/agent; name collisions
  ...
  **Channel messaging** (experimental, `experimental.agentChannels`): the main agent uses the Channel tool
  to create channels / add and remove members (members are direct subagents; the main agent joins automatically as `main`), members speak
  with `SendMessage(to: "#room")` — messages enter every member's context (same order), senders stamped by the runtime; in serial channels a lagging
  ...
  in a turn woken by a channel message, the body text goes back to the hub — **only a message addressed to the room can put words in it**. The rule lives in the system block, not the wake payload: compaction rewrites message history but never touches the system prompt,
  so the rule survives even after a long-running member's context is summarized away.
  ↑/↓ belong to the composer's prompt history; what exists and what is running is the conversation bar's job, not a second strip's. Ctrl+B opens the main-view background-agent manager (↑/↓ select, Enter detail, x stop, Esc close) — unless a shell command is running in the foreground, in which case it backgrounds that command instead; detail shows prompt/status/elapsed/tokens/tool count/recent tool activity, opens that agent's DM with Enter, and opens its perspective page with tab (one of the three doors — see the perspective page below).
  **Conversations (D89)**: one terminal, one flow, one conversation at a time. The hub (you and the model), a DM with a running subagent and a room are all the same shape and share one composer, one key surface, one approval path and one transcript renderer — there is no separate screen and no second set of controls. **Entering** one is `/open @agent` / `/open #room` / `/open hub` (Tab completes from the conversations that exist; the command is instant, so it works mid-turn), Enter on an agent in the Ctrl+B manager, or Enter on a member or a room in the team directory. **Esc returns to the hub** — navigation before interruption, so a running turn survives the press and its Esc-to-interrupt is reachable from the hub; Ctrl+C still interrupts from anywhere. **Switching** stashes the draft you were writing in the conversation you left, restores that conversation's own draft, prints a `── @name ──` / `── #name ──` rule into the flow and replays that conversation's last eight messages; from then on only it prints live. Everything else keeps accumulating in the store it already lives in and counts up an unread badge — nothing is buffered on the user's behalf. Returning prints `── hub ──` followed by whatever the hub finished while they were away. Because scrollback is written once and never rewritten, repeated excursions leave a conversation on screen more than once; the rules mark it, and Ctrl+O (the transcript view) is still the complete record of the hub session. **Sending** goes to the active conversation: a DM delivers to that instance under the user's name (same immediate dispatch path as SendMessage, shown at once and marked pending only until the receiver claims it), a channel posts as `user` (the same delivery path an agent's room message takes, members woken normally; rendering counts as read, so serial never bounces the user), and a room posts as `user` only if you are a member of it — a room you are merely watching refuses and says `read-only · /join to speak in this room`. **None of these start a model turn**, and a DM message never steers or queues behind one. Slash commands are the exception and act on the application from any conversation. A DM in flight shows its state in the redrawn tail — the sent message, the agent's tool/reasoning steps as dim lines, and a `⠙ name is replying…` row carrying the instance's live token rate — and none of that reaches scrollback, because only the record is printed. **Conversation chrome (D90)**: a one-row conversation bar on the window's last row, below the composer (D97), lists the conversations you are in (hub, your rooms, DMs) with presence for DMs, unread counts, a distinct emphasis when a conversation named the user, and the active one accented; it renders only when more than one conversation exists and elides to `…` around the active entry when the width runs out. `Ctrl+K` opens the switcher over every conversation — recency order with the hub pinned, fuzzy-filtered as you type, Enter switches, `Ctrl+X` stops the highlighted agent through the Ctrl+B manager's own stop path, Esc peels only the switcher — and it is inert behind a permission dialog. From the **hub only**, a message opening with a known conversation's name (`@agent ` / `#channel `, matched exactly against the registry) delivers the rest there without switching and leaves a dim `→ @name: …` receipt in the flow; an unknown name is ordinary prose and starts a normal turn. Inside a conversation there is no line-leading routing, because the buffer already is the target. `/team` output goes to the team directory's recent feed rather than the hub's info tier, and the hub says `→ team (ctrl+t)` unless the directory is already open. While a DM is active the composer's border and `❯` glyph, and the `is replying…` row, wear that agent's palette colour.
  **Rooms and the team directory (D95)**: a **room** is the only group conversation there is, and its members are any subset of the team — it does not have to include you, and agents form rooms among themselves with the `Channel` tool (creating one seats the creator and nobody else; `user` and `main` are seated only when named). A room you are in is one of your conversations: it is in the bar, in `Ctrl+K`, and you speak in it by typing. A room you are **not** in is still visible — the team directory lists it, marked `you're not in`, and Enter opens it **read-only**: the same flow, framed `── #name · observer · read-only ──`, with a standing `read-only · /join to speak in this room` hint under the composer and a refusal (not an error) if you submit anyway. `/join` — or `j` on the room in the directory — makes you a member; joining and leaving are **written into the room where every member sees them**, as dim `· user joined · 14:32` lines, so there is no quiet way to lurk and then speak. `/leave` is the counterpart, and a room you leave drops out of the bar while staying readable. Roster changes never wake anybody and never make a serial sender stale.
  **The team is not a conversation** — it is a directory, on the second stop of `Ctrl+T` (tasks → team → closed; Esc closes one stop). It shows the roster with presence and the rooms each member is in, every room with its members and a `you're not in` mark, and the last ten lifecycle events (spawn/done, plus `/team` output). **Main is on the roster, first** (D100): it is a participant like the rest — presence is the host turn, its label is `main · console · idle`, its rooms list like anyone's — and Enter on its row goes to the console, not to a DM with an instance that does not exist. ↑/↓ move, Enter opens a member's conversation or a room, `o` opens the member's perspective page, `j` joins the room under the cursor; chords pass through, so `Ctrl+T` closes it and `Ctrl+K` still reaches the switcher. It navigates and informs only — stopping an agent stays in the `Ctrl+B` manager, which owns that verb and its warning, and main is not stoppable at all.
  **The perspective page (D96, every participant since D100)**: one participant's communications, read-only, from its own point of view — a two-level page on the alternate screen with the transcript's scrolling and `/` search, grouping a merged `timeline`, `direct messages` one row per counterpart, `rooms`, and `intake` (Enter opens a thread, Esc walks back, `q` closes). **Main has one too**, built from the session transcript: the console conversation as its `@user` lane, one lane per agent that wrote to it, its rooms, its dispatch notifications as intake; the attribution default flips with the protagonist (unmarked prose is main in a subagent's record, you in main's), and a transcript's only clock is the turn marker, so times there are turn times and a pre-marker session shows none. **Three doors**: `tab` on an **empty composer** in a conversation (the agent's page in `@agent`, main's in the console; a `#room` has no single protagonist and the key does nothing — with text in the composer `tab` is still completion), `o` on a member in the `Ctrl+T` directory, and `tab` on an agent in the `Ctrl+B` detail. It is a snapshot: reopening is the refresh, and nothing on it can be typed into.
  In a DM or a channel the sender's name heads each run of messages (grouped: consecutive messages from one speaker carry it once), and every message carries its send time on its own first row, right-aligned and muted, at least two columns clear of the text — `HH:MM`, dated `M/D HH:MM` once it is not from today — the same stamp the main transcript uses; where the row is too narrow to hold both, the stamp is dropped rather than the message wrapped. DM replays reuse the main transcript's user bubbles, assistant markdown, prefixes, wrapping and row structure, because they *are* the main transcript's row builder.
  **`@agent` is the pure pair (D99)**: what you said, what it answered *you*, and the work it did for those turns — nothing else. The task it was created with, main's instructions to it, room relays, mail from other agents, the chases for its silence and the task reminder are all somebody else's conversation and live on its perspective page; they no longer render here as dim scaffolding lines. Its work comes back the way @main's does — one collapsed activity group per run (`⏺ Searched for 4 patterns, read 2 files`) hung on the agent's own message, not a column of `⏺ Tool(…)` lines. And the live tail belongs to **your** run: when the agent is working for main, for a room or on a chase, the DM shows no stream and no `is replying…` row, because no reply is owed to you. The `[DM from user]` marker is transport rather than text and never renders. An agent main spawned that you never wrote to therefore has an **empty** pair view, and it says so (D100): the conversation opens with its rule plus one dim `· no conversation yet · tab opens @scout's record ·`, which is not repeated while it is already the last thing that conversation printed.
  **Unread (D99)**: a DM's badge counts messages, not history rows, so a turn that made forty tool calls counts as one reply; it wears the mention accent only when the agent actually answered you. `@main` has a badge of its own — main speaking while you are in another conversation counts, and a `⚠ @name · …` failure alert counts *and* wants you. A room says your name case-insensitively (`@User`, `@USER,` reach you; `@username` does not).
  **Avatars**: on terminals that can place kitty images (the same capability that renders inline images), each sender gets one of eight bundled anime-style portraits, 4×2 cells beside the name; elsewhere it falls back to the sender's initial on a colour, and the row count is identical either way. A team member's portrait is pinned in `.bingo/team.json` (`"avatar": "sora"`), so a crew keeps a fixed cast; everyone else gets a face derived from their name. **Every conversation wears the face in a left gutter** (D97, extended to the hub in D99) — four or five cells taken out of the width before the body wraps, the portrait on the first row of each sender's run and blank on every row after it, with work steps and system lines taking the indentation and no face. The main agent has a **reserved portrait**: it is not in the vocabulary `team.json` may pin and the name hash never lands on it, so the console looks the same in every session and no teammate can be mistaken for it. A terminal that purges its image store (a resize) redraws the faces still on screen; rows already in scrollback leave blank columns where the portrait was. The sender name still heads each run of messages in a DM or a room, because with more than two speakers the name is not decoration; @main has no name rows, because it has exactly two. `experimental.chatAvatars` (off by default) governs one remaining thing: whether a subagent's watch row wears that agent's portrait instead of its `◉`.
