...
/// Headless hooks whose barrier reaches the session's real input queue, holding
/// `items`: whether the queue is empty afterwards is whether the barrier took them.
async fn steering_hooks(session: &Arc<Session>, items: &[&str]) -> EngineHost {
    for text in items {
        session
            .queue
            .enqueue(Enqueue {
    ...
    ui.requests.steer = Arc::new(move || {
        let queue = queue.clone();
        Box::pin(async move { queue.absorb(ConvKey::Main, TurnId::new("turn_1")).await })
    });
    ui
}

/// Whether anything is still the composer's.
fn still_queued(session: &Arc<Session>) -> bool {
    !session.queue.of(&ConvKey::Main).is_empty()
}
...
async fn steering_lands_after_the_tool_results_of_the_same_message() {
    ...
    let ui = steering_hooks(&session, &["use tabs"]).await;
    ...
    assert!(!still_queued(&session), "the barrier took it");
}
...
async fn a_turn_without_tools_never_reaches_a_barrier() {
    ...
    let session = test_session(base_url, None);
    let ui = steering_hooks(&session, &["wait"]).await;
    run_query(&session, Vec::new(), "go", &[], &ui, None)
        .await
        .unwrap();
    assert!(
        still_queued(&session),
...
async fn an_interrupted_turn_takes_nothing_from_the_queue() {
    ...
    let session = test_session(base_url, None);
    let ui = steering_hooks(&session, &["stop that"]).await;
