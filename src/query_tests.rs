//! Query-loop tests.
//!
//! Split out of `query.rs` (D130) for the same reason `query_steer_tests.rs`
//! was: the file is at its line cap and the tests are what pushed it over. The
//! module is `query::tests` exactly as before — `use super::*` still reaches
//! the loop's own items, so nothing here changed but the file it lives in.
...
fn a_session_allow_is_installed_once_and_stays_in_memory() {
    ...
    assert!(
        permission_snapshot(&rules).deny.is_empty() && permission_snapshot(&rules).ask.is_empty()
    );
}
...
fn unclosed_thinking_turn(thinking: &str) -> String {
    sse(&[
        ...
        (
            "content_block_delta",
            format!(r#"{{"index":0,"delta":{{"type":"thinking_delta","thinking":"{thinking}"}}}}"#),
        ),
        (
            "message_delta",
...
async fn anthropic_overflow_compacts_and_retries_once() {
    let base_url = spawn_anthropic_api(vec![
        ...
        ApiResponse::Ok(r#"{"content":[{"type":"text","text":"compacted context"}]}"#.to_string()),
...
async fn declared_max_tokens_reaches_the_request() {
    let (base_url, log) =
        spawn_anthropic_api_counting(10, vec![ApiResponse::Ok(text_turn("hi", "end_turn"))]).await;
...
fn agent_summary_uses_description_to_distinguish_parallel_agents() {
    ...
    let b =
        serde_json::json!({"background": true, "description": "audit mechanism", "prompt": "..."});
...
async fn interrupted_stream_records_the_partial_reply_and_the_marker() {
    let home = std::env::temp_dir().join(format!("bingo-interrupt-partial-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&home);
    std::fs::create_dir_all(&home).unwrap();
    let transcript = crate::transcript::create(&home, &home).unwrap();
...
async fn interrupt_before_any_content_records_the_marker_alone() {
    let home = std::env::temp_dir().join(format!("bingo-interrupt-silent-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&home);
    std::fs::create_dir_all(&home).unwrap();
    let transcript = crate::transcript::create(&home, &home).unwrap();
    let outcome = interrupted_turn(
