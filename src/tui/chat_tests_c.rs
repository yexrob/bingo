...
fn grouped_results_keep_their_output_for_expansion() {
    ...
    let group = chat.conv.messages[0].groups.first().expect("one group");
    assert_eq!(group.activities.len(), 3, "all three folded together");
    for &idx in &group.activities {
        let member = &chat.conv.messages[0].activities[idx];
...
fn a_large_grouped_result_is_bounded_like_a_standalone_one() {
    ...
    let content = |standalone: bool| -> Vec<String> {
        ...
        chat.drain_events();
        chat.conv.messages[0].activities[0]
            .content
            .iter()
            .map(|l| l.plain_text())
            .collect()
...
fn an_interrupted_group_member_still_has_nothing_to_expand() {
    ...
    let stopped = &chat.conv.messages[0].activities[1];
    assert!(
        !stopped.expandable(),
        "an interrupted call produced no output to keep"
    );
    assert!(
        chat.conv.messages[0].activities[0].expandable(),
...
fn a_waiting_permission_prompt_rings() {
    ...
    let _verdict = chat.open_test_permission("Bash", "cargo build", Some("cargo build"), None);
    assert!(chat.pending_ask.is_some(), "the request is accepted");
    assert_eq!(
        emitted(&mut chat),
    ...
    let _second = chat.open_test_permission("Bash", "cargo test", Some("cargo test"), None);
...
fn esc_peels_one_layer_per_press_before_it_reaches_the_turn() {
    ...
    while let Some(layer) = chat.esc_layer() {
        ...
        assert!(chat.conv.busy, "the turn kept running through {layer:?}");
    }
    assert_eq!(
        order,
        vec![
    ...
    assert!(chat.conv.interrupted, "the last press reached the turn");
}
...
fn esc_over_a_busy_turn_closes_the_dropdown_and_says_so() {
    ...
    assert!(
        chat.conv.busy && !chat.conv.interrupted,
    ...
    assert!(
        chat.conv.interrupted,
        "with nothing left open, Esc interrupts"
    );
}
...
fn an_interrupt_settles_the_dialog_it_leaves_behind() {
    ...
    assert!(
        chat.conv.interrupted,
        "ctrl+c still interrupts through a dialog"
    );
    ...
    // The keys that answered the dialog are ordinary composer keys again.
...
fn turn_end_settles_the_asks_whose_turn_is_gone() {
    ...
    assert!(
        chat.pending_ask
            .as_ref()
            .is_some_and(|(_, request)| request.title == "Allow running Edit"),
        "and surfaces next"
    );
}
...
/// Drive a real permission prompt through the gate's own path: the run asks,
/// the core holds the answer, and the console picks it up from the view.
async fn gate_asks(
    chat: &mut Chat,
    tool: &str,
    reason: &str,
    ...
    scope: Option<&str>,
    diff: Option<&str>,
) -> tokio::task::JoinHandle<AskOutcome> {
    let ask = crate::app::interaction::permission_ask(
        chat.session.interactions.clone(),
        crate::ui::ConvKey::Main,
    );
    let pending = ask(&crate::query::AskContext {
        tool,
        reason,
        input,
    ...
    panic!("the prompt never reached the console");
}

/// A dialog whose type-ahead guard is already behind it.
fn past_guard() -> std::time::Instant {
    std::time::Instant::now() + crate::app::interaction::CONFIRM_GUARD
}
...
async fn the_approval_prompt_offers_the_three_options_verbatim() {
    ...
    assert_eq!(
        verdict.await.unwrap_or_else(|error| panic!("{error}")),
        AskOutcome::Allow
    );
    assert!(
...
async fn enter_inside_the_guard_window_answers_nothing() {
    ...
    let now = opened + crate::app::interaction::CONFIRM_GUARD;
...
async fn shift_tab_takes_the_session_option() {
    ...
    .await;
    chat.ask_focus = 2;
    // shift+tab confirms, so D81's guard covers it exactly as it covers a digit
    // and Enter: the core holds back every keyboard approval, not a chosen few.
    assert!(chat.on_key_at(KeyCode::BackTab, KeyModifiers::empty(), past_guard()));
    assert_eq!(
        verdict.await.unwrap_or_else(|error| panic!("{error}")),
...
async fn ask_user_question_keeps_its_own_shape() {
    ...
    assert!(
        chat.pending_ask.is_some(),
        "the question reached the console"
    );
    let flow = visible(&mut chat, 100, 40);
    ...
    assert_eq!(
        answer.await.unwrap_or_else(|error| panic!("{error}")),
