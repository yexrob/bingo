...
- v1.28 (2026-08-09): the workspace stops quoting the runtime at the user (D49). Wake-up scaffolding written into an instance's history — a relayed channel message, a follow-up chase, the `[SYSTEM NOTIFICATION - TASK REMINDER]` block — used to render as a full quoted message under a "你" name row and avatar, as if the user had typed it. It now collapses to a single dim `▏` line (`#dev-team · main: …`, `系统提醒 · 任务工具`) that owns no name row and does not split the grouping of the real messages around it; text a person actually wrote is untouched. Same tier and same visual language as a tool attachment: this is context, not conversation.
