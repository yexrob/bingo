...
- v1.9 (2026-08-08): closed the real alternate-screen host gap: `run_fullscreen` now consumes a testable fullscreen frame assembler that branches to the same Full error screen as inline mode, hides prompt/caret, and preserves Enter retry / Esc return semantics. Added `fullscreen_frame_presents_full_error_and_hides_prompt`; F1/F3/D2/D3 now cover the actual fullscreen presentation seam, not only `Frame::assemble`. Idle slash-error feedback now keeps the host ticker awake through its 8s TTL, then clears and returns to idle.
