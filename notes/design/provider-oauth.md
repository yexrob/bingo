...
### D34. Built-in official provider presets (zero-config subscriptions)

**Status: draft — pending main's confirmation of §6.5 (2026-08).**

**Problem**: official subscriptions (codex/ChatGPT, opencode-go) require hand-editing `settings.json` (`"codex": {protocol, oauth.kind, ...}`). Reference CLIs are install-and-use — the subscription template must ship inside bingo and the user only logs in.

**Decisions**:

1. **Preset table as compile-time consts** (`src/api/providers/presets.rs`): `codex` (openai + `OpenAiVariant::Codex` + `https://chatgpt.com/backend-api` + `oauth.kind: codex` + gpt-5.x whitelist) and `opencode-go` (openai + Default variant + `https://opencode.ai/zen/go` + apiKey + gpt-5.6-luna whitelist). Zero config to be visible and loginable.
