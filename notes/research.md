...
- **Palette**: aubergine is kept in both terminal themes (it is the single most recognisable thing about the product) while the conversation pane follows the terminal, so a dark terminal doesn't get a white slab. Non-truecolor terminals bring the whole skin down through the shared `theme::to_ansi256`.
...
### D44. Acknowledgement chasing: the sender's wait, bounded follow-ups, and a report

- **Problem**: D41 gave every direct message a `MsgId` and an `Ack`, but the mechanism was pull-only. `AgentControl(action=messages)` never pushes; the hub has to remember to look. A message that stays `Queued` — receiver busy through several boundaries, or a hub turn that ended right after `SendMessage` without a flush — therefore fails silently: the sender saw "queued", nothing contradicted it, and the miss surfaces only if the model happens to re-check. Automating that check by prompting ("remember to poll") puts a correctness property in the least reliable layer available.
- **Decision — the sender names the wait, the harness keeps the clock**: `SendMessage(ack_timeout: <seconds>)`, clamped to 5–3600, is the whole opt-in surface. A per-message tokio task (`spawn_ack_watchdog`) sleeps that long and re-reads the same delivery record the tool reports, so the automatic check and the manual one can never disagree. Omitting the parameter leaves the previous path untouched, watchdog and all.
- **Decision — chase, then stop**: while the record still says `Queued`, each round appends `InboxItem::FollowUp` to the receiver's inbox and re-runs `flush_agent_inbox`. The flush is the half that actually repairs the stranded-at-a-boundary case (an idle instance with nobody to claim it); the follow-up is the half that reaches a receiver already running, riding along with the original message it names, since queueing is the only channel to a busy instance. `MAX_FOLLOW_UPS = 3` bounds it: a retry budget that never runs out is not a mechanism, it is a loop. `AgentRegistry::follow_up` does the read and the enqueue under the single registry lock, so a flush racing the watchdog cannot turn a just-delivered message into a pointless nudge.
- **Decision — silence only when nothing went wrong**: the watchdog registers no watch line up front. On the first missed deadline it registers one, and every non-clean outcome ends terminally — `Done` when the follow-ups worked (with the round that finally carried it), `Failed` when the message was dropped, the instance was removed, or the budget ran out. Terminal watch states are already the path into the hub's next turn (D41 addressed notifications), so the report costs no new plumbing. An on-time delivery leaves no line, no notification, and no trace beyond the ack record.
- **Boundary**: an acknowledgement still means "folded into the receiver's prompt", not "acted upon" — delivery ≠ reply, and this mechanism does not pretend otherwise. Follow-ups carry no acknowledgement of their own (they are not user messages and take no `MsgId`), the retry never re-sends the original text, and nothing here changes hub-and-spoke gating: `SendMessage` remains depth-0, so only the hub can arm a watchdog. Channel deposits and `/team assign` deliver unchanged, with no wait to name.
- **Acceptance**: 822 tests pass with fmt/check/clippy `-D warnings` clean. Focused regressions cover the bounded chase (three follow-ups then `Exhausted`, follow-ups batched with the original, the ack recording its wait and round count), settling on a dropped message and a removed instance, and both driver halves under a paused clock — an unacknowledged message chased three times and then reported to the hub, and an acknowledged one leaving no notification and no watch line.
